"""Mid BTC-USDT 1D EMA12/30 long-only — Core-style RETURN gate at Mid €40.

LOCKED #51. SAME EMA12/30 long/flat rule as mid_doge_ema_coregate (#41) /
mid_btc_ema_4h (#49) / EmaTrendV1 / walk_long_flat, on **BTC-USDT 1D**
(daily Core-rule on BTC Mid; ≠ 4H Mid #49).
...
from atlas.strategy.mid_btc_ema_1d import BAR, FAMILY, FAST, SLOW

SOURCE = "mid_btc_ema_1d"
SPOT_MD = "BTC-USDT"
...
SAME_RULE_AS = "mid_doge_ema_coregate (#41) / mid_btc_ema_4h (#49) / EmaTrendV1 — BTC 1D daily Mid ≠ 4H #49"


def resolve_windows(set_id: str, *, data_dir: Path, rest_base: str, pause_s: float) -> list[CascadeWindow]:
    """Same A/B calendars as recent Mid trials; probe BTC 1D MD; label fallbacks."""
    if set_id == "A":
...
def _holdout_window(window: CascadeWindow, hold_bars: list[Bar]) -> CascadeWindow:
    if not hold_bars:
        raise ReplayError("empty holdout for mid btc ema 1d")
...
def run_mid_ema_slice(
    *,
    ...
    """Full-sleeve EMA12/30 long/flat on Mid equity (BTC-USDT 1D)."""
...
def evaluate_window(
    ...
    *,
    ...
    mid_full = run_mid_ema_slice(
        ...
        fee_rate=fee_rate,
        slippage_bps=slip,
        label=f"mid-ema1d-full-{window.id}",
    )
    mid_hold = None
    if hold:
        ...
        mid_hold = run_mid_ema_slice(
            ...
            fee_rate=fee_rate,
            slippage_bps=slip,
            label=f"mid-ema1d-hold-{window.id}",
        )
    ...
    return {
        ...
        "mid": {
            ...
            "differs_from_holdout_exp_gate": True,
            "differs_reason": DIFFERS_REASON,
            "tf_note": "daily Core-rule on BTC Mid 1D ≠ 4H Mid #49",
        },
        "scalp": {
            "out_of_trial": True,
            "note": "Scalp halted for this trial — Mid BTC EMA 1D Core-style return gate only",
        },
...
def render_set_markdown(bundle: dict[str, Any]) -> str:
    ...
    lines.append(f"# Mid #51 BTC EMA 4H — set {sid}")
    lines.append("")
...
def render_markdown(bundle_a: dict[str, Any], bundle_b: dict[str, Any] | None) -> str:
    lines: list[str] = []
    lines.append("# 51 — Mid BTC EMA12/30 on **1D** Core-style RETURN gate (Mid sleeve; Scalp OUT)")
    lines.append("")
    ...
    lines.append(
        "**Family:** Mid **EMA12/30 long/flat** on **BTC-USDT 1D** — SAME Core rule "
        f"(`EmaTrendV1` / `walk_long_flat` / #41/#45/#49 spirit) at Mid €40. **Daily Core-rule on BTC** (≠ 4H Mid #49). "
        "**Gate:** Core-style RETURN (intentional). "
        f"**`differs_from_holdout_exp_gate: true`** — reason: {DIFFERS_REASON}. Scalp halted. **Live:** BTC = **research-only** for live."
    )
    lines.append("")
    ...
    lines.append(f"- Bar: **{BAR}** (daily Core-rule on BTC Mid; ≠ 4H #49).")
    lines.append("- Low n_trades: **OK** (document n_trades / TIM / expectancy; not a FAIL gate).")
    lines.append("- Windows: same A/B calendars as recent Mid trials mapped to BTC 1D bars; MD fallbacks labeled.")
    lines.append("")
    ...
    lines.append(
        ...
        "Mirrors scoring spirit of mid_doge_ema_coregate (#41) / mid_doge_ema_4h (#45) / mid_btc_ema_4h (#49) adapted to this BTC **1D** trial. "
