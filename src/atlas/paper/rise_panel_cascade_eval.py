...
from atlas.strategy.scalp_doge_donchian_1h import (
    BAR as SCALP_BAR_DONCHIAN_1H,
    FAMILY as SCALP_FAMILY_DONCHIAN_1H,
    ScalpDogeDonchian1hV1,
)
from atlas.strategy.scalp_doge_breakout_1h import (
    BAR as SCALP_BAR_BREAKOUT_1H,
    FAMILY as SCALP_FAMILY_BREAKOUT_1H,
    ScalpDogeBreakout1hV1,
)
from atlas.strategy.scalp_doge_ema1221_1h import (
    BAR as SCALP_BAR_EMA1221_1H,
    FAMILY as SCALP_FAMILY_EMA1221_1H,
    ScalpDogeEma1221V1,
)
from atlas.strategy.scalp_doge_dual_thrust_1h import (
    BAR as SCALP_BAR_DUAL_THRUST_1H,
    FAMILY as SCALP_FAMILY_DUAL_THRUST_1H,
...
COMPOUND_ID_SCALP_DONCHIAN = "rise_panel_v1_cascade_compound_721_scalp_donchian_1h"
COMPOUND_ID_SCALP_BREAKOUT = "rise_panel_v1_cascade_compound_721_scalp_breakout_1h"
COMPOUND_ID_SCALP_EMA1221 = "rise_panel_v1_cascade_compound_721_scalp_ema1221_1h"
COMPOUND_ID_SCALP_DUAL_THRUST = "rise_panel_v1_cascade_compound_721_scalp_dual_thrust_1h"
...
SCALP_CANDIDATE_ID_DONCHIAN = "rise_panel_v1_scalp_doge_donchian20_10_1h_eur20"
SCALP_CANDIDATE_ID_BREAKOUT = "rise_panel_v1_scalp_doge_breakoutv1_1h_eur20"
SCALP_CANDIDATE_ID_EMA1221 = "rise_panel_v1_scalp_doge_ema12_21_1h_eur20"
SCALP_CANDIDATE_ID_DUAL_THRUST = "rise_panel_v1_scalp_doge_dual_thrust_n20_k0505_1h_eur20"
...
SCALP_DONCHIAN_LABEL = "scalp_doge_donchian_1h"
SCALP_BREAKOUT_LABEL = "scalp_doge_breakout_1h"
SCALP_EMA1221_LABEL = "scalp_doge_ema1221_1h"
SCALP_DUAL_THRUST_LABEL = "scalp_doge_dual_thrust_1h"
...
SCALP_MODES = ("1h_daily_bull", "4h_ema", "15m_ema", "1h_rsi_mr", "1h_rsi_ema1221", "1h_donchian", "1h_breakout", "1h_ema1221", "1h_dual_thrust")
...
def run_scalp_rsi_mr_1h_on_window(
    *,
    bars_1h: list,
    window: RiseWindow,
    equity: float,
    ...
    """Scalp DOGE 1H RSI(14) MR long/flat on one rise window (#59 substitute)."""
    settings = EmaBookSettings(
        equity_eur=float(equity),
        fee_rate=float(fee_rate),
        slippage_bps=float(slippage_bps),
    ...
    strat = ScalpDogeRsiMr1hV1()
    trade_bars = [b for b in bars_1h if window.start_ms <= b.ts_open_ms < window.end_ms_exclusive]
    if len(trade_bars) < 12:
        return _fail_row(window, arm="scalp_rsi14_mr_1h", error="insufficient 1H bars")
    walk = walk_long_flat(
        bars_1h,
        strategy=strat,
        settings=settings,
    ...
    return {
        ...
        "character": window.character,
        "asset": ASSET,
        "bar": SCALP_BAR_RSI_1H,
        "arm": "scalp_rsi14_mr_1h",
        "family": SCALP_FAMILY_RSI_1H,
        "equity_eur": equity,
        "n_trades": int(walk.get("n_trades") or 0),
        "n_entries": int(walk.get("n_entries") or 0),
        "expectancy_after_costs_eur": walk.get("expectancy_after_costs_eur"),
...
def run_scalp_donchian_1h_on_window(
    *,
    bars_1h: list,
    window: RiseWindow,
    equity: float,
    ...
    """Scalp DOGE 1H Donchian 20/10 long/flat on one rise window (#61 substitute)."""
    settings = EmaBookSettings(
        equity_eur=float(equity),
        fee_rate=float(fee_rate),
        slippage_bps=float(slippage_bps),
    ...
    trade_bars = [b for b in bars_1h if window.start_ms <= b.ts_open_ms < window.end_ms_exclusive]
    if len(trade_bars) < 12:
        return _fail_row(window, arm="scalp_donchian20_10_1h", error="insufficient 1H bars")
    walk = walk_long_flat(
        bars_1h,
        strategy=strat,
        settings=settings,
    ...
    return {
        ...
        "character": window.character,
        "asset": ASSET,
        "bar": SCALP_BAR_DONCHIAN_1H,
        "arm": "scalp_donchian20_10_1h",
        "family": SCALP_FAMILY_DONCHIAN_1H,
        "equity_eur": equity,
        "n_trades": int(walk.get("n_trades") or 0),
        "n_entries": int(walk.get("n_entries") or 0),
        "expectancy_after_costs_eur": walk.get("expectancy_after_costs_eur"),
...
def run_scalp_breakout_1h_on_window(
    *,
    bars_1h: list,
    window: RiseWindow,
    equity: float,
    ...
    """Scalp DOGE 1H BreakoutV1 long/flat on one rise window (#62 substitute)."""
    settings = EmaBookSettings(
        equity_eur=float(equity),
        fee_rate=float(fee_rate),
        slippage_bps=float(slippage_bps),
    ...
    trade_bars = [b for b in bars_1h if window.start_ms <= b.ts_open_ms < window.end_ms_exclusive]
    if len(trade_bars) < 12:
        return _fail_row(window, arm="scalp_breakout_v1_1h", error="insufficient 1H bars")
    walk = walk_long_flat(
        bars_1h,
        strategy=strat,
        settings=settings,
    ...
    return {
        ...
        "character": window.character,
        "asset": ASSET,
        "bar": SCALP_BAR_BREAKOUT_1H,
        "arm": "scalp_breakout_v1_1h",
        "family": SCALP_FAMILY_BREAKOUT_1H,
        "equity_eur": equity,
        "n_trades": int(walk.get("n_trades") or 0),
        "n_entries": int(walk.get("n_entries") or 0),
        "expectancy_after_costs_eur": walk.get("expectancy_after_costs_eur"),
...
def run_scalp_ema1221_1h_on_window(
    *,
    bars_1h: list,
    window: RiseWindow,
    equity: float,
    ...
    """Scalp DOGE 1H EMA12/21 long/flat on one rise window (#63 substitute)."""
    settings = EmaBookSettings(
        equity_eur=float(equity),
        fee_rate=float(fee_rate),
        slippage_bps=float(slippage_bps),
    ...
    trade_bars = [b for b in bars_1h if window.start_ms <= b.ts_open_ms < window.end_ms_exclusive]
    if len(trade_bars) < 12:
        return _fail_row(window, arm="scalp_ema12_21_1h", error="insufficient 1H bars")
    walk = walk_long_flat(
        bars_1h,
        strategy=strat,
        settings=settings,
    ...
    return {
        ...
        "character": window.character,
        "asset": ASSET,
        "bar": SCALP_BAR_EMA1221_1H,
        "arm": "scalp_ema12_21_1h",
        "family": SCALP_FAMILY_EMA1221_1H,
        "equity_eur": equity,
        "n_trades": int(walk.get("n_trades") or 0),
        "n_entries": int(walk.get("n_entries") or 0),
        "expectancy_after_costs_eur": walk.get("expectancy_after_costs_eur"),
...
def run_scalp_dual_thrust_1h_on_window(
    *,
    bars_1h: list,
    window: RiseWindow,
    equity: float,
    ...
    """Scalp DOGE 1H Dual Thrust N=20 k1=k2=0.5 long/flat on one rise window (#83)."""
    settings = EmaBookSettings(
        equity_eur=float(equity),
        fee_rate=float(fee_rate),
        slippage_bps=float(slippage_bps),
    ...
    trade_bars = [b for b in bars_1h if window.start_ms <= b.ts_open_ms < window.end_ms_exclusive]
    if len(trade_bars) < 12:
        return _fail_row(window, arm="scalp_dual_thrust_1h", error="insufficient 1H bars")
    walk = walk_long_flat(
        bars_1h,
        strategy=strat,
        settings=settings,
    ...
    return {
        ...
        "character": window.character,
        "asset": ASSET,
        "bar": SCALP_BAR_DUAL_THRUST_1H,
        "arm": "scalp_dual_thrust_1h",
        "family": SCALP_FAMILY_DUAL_THRUST_1H,
        "equity_eur": equity,
        "n_trades": int(walk.get("n_trades") or 0),
        "n_entries": int(walk.get("n_entries") or 0),
        "expectancy_after_costs_eur": walk.get("expectancy_after_costs_eur"),
...
def run_cascade_compound_panel(
    ...
    *,
    ...
    scalp_mode:
      ...
      - "1h_donchian": Scalp #61 (DOGE 1H Donchian 20/10 €20)
      - "1h_breakout": Scalp #62 (DOGE 1H BreakoutV1 long/flat €20)
      - "1h_ema1221": Scalp #63 (DOGE 1H EMA12/21 long/flat €20)
      - "1h_dual_thrust": Scalp #83 (DOGE 1H Dual Thrust N=20 k1=k2=0.5 €20)
    """
    ...
    for w in panel_windows():
        ...
        try:
            if scalp_mode == "4h_ema":
                ...
                bars_1h = fetch_bars(
                    cw, ASSET, SCALP_BAR_RSI_1H, data_dir=data_dir, rest_base=rest_base,
                    pause_s=pause_s, pad_days=WARMUP_PAD_1H_DAYS,
                )
                scalp = run_scalp_rsi_mr_1h_on_window(
            ...
            elif scalp_mode == "1h_donchian":
                bars_1h = fetch_bars(
                    cw, ASSET, SCALP_BAR_DONCHIAN_1H, data_dir=data_dir, rest_base=rest_base,
                    pause_s=pause_s, pad_days=WARMUP_PAD_1H_DAYS,
                )
                scalp = run_scalp_donchian_1h_on_window(
                    bars_1h=bars_1h, window=w,
                    equity=SCALP_START_EUR, fee_rate=fee_rate, slippage_bps=slip,
                )
            elif scalp_mode == "1h_breakout":
                bars_1h = fetch_bars(
                    cw, ASSET, SCALP_BAR_BREAKOUT_1H, data_dir=data_dir, rest_base=rest_base,
                    pause_s=pause_s, pad_days=WARMUP_PAD_1H_DAYS,
                )
                scalp = run_scalp_breakout_1h_on_window(
                    bars_1h=bars_1h, window=w,
                    equity=SCALP_START_EUR, fee_rate=fee_rate, slippage_bps=slip,
                )
            elif scalp_mode == "1h_ema1221":
                bars_1h = fetch_bars(
                    cw, ASSET, SCALP_BAR_EMA1221_1H, data_dir=data_dir, rest_base=rest_base,
                    pause_s=pause_s, pad_days=WARMUP_PAD_1H_DAYS,
                )
                scalp = run_scalp_ema1221_1h_on_window(
                    bars_1h=bars_1h, window=w,
                    equity=SCALP_START_EUR, fee_rate=fee_rate, slippage_bps=slip,
                )
            elif scalp_mode == "1h_dual_thrust":
                bars_1h = fetch_bars(
                    cw, ASSET, SCALP_BAR_DUAL_THRUST_1H, data_dir=data_dir, rest_base=rest_base,
                    pause_s=pause_s, pad_days=WARMUP_PAD_1H_DAYS,
                )
                scalp = run_scalp_dual_thrust_1h_on_window(
                    bars_1h=bars_1h, window=w,
                    equity=SCALP_START_EUR, fee_rate=fee_rate, slippage_bps=slip,
                )
            else:
            ...
            if scalp_mode == "4h_ema":
                ...
                arm_fail = "scalp_donchian20_10_1h"
            elif scalp_mode == "1h_breakout":
                arm_fail = "scalp_breakout_v1_1h"
            elif scalp_mode == "1h_ema1221":
                arm_fail = "scalp_ema12_21_1h"
            elif scalp_mode == "1h_dual_thrust":
                arm_fail = "scalp_dual_thrust_1h"
            else:
    ...
    return {
        ...
        "systems": {
            ...
            "scalp": (
                ...
                if scalp_mode == "15m_ema"
                    ...
                    "id": SCALP_CANDIDATE_ID_RSI_MR,
                    "bar": SCALP_BAR_RSI_1H,
                    "strategy": SCALP_FAMILY_RSI_1H,
                    "sleeve_start_eur": SCALP_START_EUR,
                    "standin_label": SCALP_RSI_MR_LABEL,
                    "prefer": "1H RSI(14) MR long/flat at Scalp €20 (#59)",
                    "provisional_scalp": provisional_scalp,
                    "soft_promote": scalp_soft,
                    "scalp_mode": scalp_mode,
                }
                ...
                if scalp_mode == "1h_rsi_ema1221"
                    ...
                    "id": SCALP_CANDIDATE_ID_DONCHIAN,
                    "bar": SCALP_BAR_DONCHIAN_1H,
                    "strategy": SCALP_FAMILY_DONCHIAN_1H,
                    "sleeve_start_eur": SCALP_START_EUR,
                    "standin_label": SCALP_DONCHIAN_LABEL,
                    "prefer": "1H Donchian 20/10 long/flat at Scalp €20 (#61)",
                    "provisional_scalp": provisional_scalp,
                    "soft_promote": scalp_soft,
                    "scalp_mode": scalp_mode,
                }
                if scalp_mode == "1h_donchian"
                else {
                    "id": SCALP_CANDIDATE_ID_BREAKOUT,
                    "bar": SCALP_BAR_BREAKOUT_1H,
                    "strategy": SCALP_FAMILY_BREAKOUT_1H,
                    "sleeve_start_eur": SCALP_START_EUR,
                    "standin_label": SCALP_BREAKOUT_LABEL,
                    "prefer": "1H BreakoutV1 long/flat at Scalp €20 (#62)",
                    "provisional_scalp": provisional_scalp,
                    "soft_promote": scalp_soft,
                    "scalp_mode": scalp_mode,
                }
                if scalp_mode == "1h_breakout"
                else {
                    "id": SCALP_CANDIDATE_ID_EMA1221,
                    "bar": SCALP_BAR_EMA1221_1H,
                    "strategy": SCALP_FAMILY_EMA1221_1H,
                    "sleeve_start_eur": SCALP_START_EUR,
                    "standin_label": SCALP_EMA1221_LABEL,
                    "prefer": "1H EMA12/21 long/flat at Scalp €20 (#63)",
                    "provisional_scalp": provisional_scalp,
                    "soft_promote": scalp_soft,
                    "scalp_mode": scalp_mode,
                }
                if scalp_mode == "1h_ema1221"
                else {
                    "id": SCALP_CANDIDATE_ID_DUAL_THRUST,
                    "bar": SCALP_BAR_DUAL_THRUST_1H,
                    "strategy": SCALP_FAMILY_DUAL_THRUST_1H,
                    "sleeve_start_eur": SCALP_START_EUR,
                    "standin_label": SCALP_DUAL_THRUST_LABEL,
                    "prefer": "1H Dual Thrust N=20 k1=k2=0.5 long/flat at Scalp €20 (#83)",
                    "provisional_scalp": provisional_scalp,
                    "soft_promote": scalp_soft,
                    "scalp_mode": scalp_mode,
                }
                if scalp_mode == "1h_dual_thrust"
                else {
        ...
        "compound_id": (
            ...
            else COMPOUND_ID_SCALP_DONCHIAN if scalp_mode == "1h_donchian"
            else COMPOUND_ID_SCALP_BREAKOUT if scalp_mode == "1h_breakout"
            else COMPOUND_ID_SCALP_EMA1221 if scalp_mode == "1h_ema1221"
            else COMPOUND_ID_SCALP_DUAL_THRUST if scalp_mode == "1h_dual_thrust"
            else COMPOUND_ID
        ),
        "provisional_scalp": provisional_scalp,
