...
Walker extends #125 exits with rsi_exit (≥67 on 3m RSI in force).
Paper only. place_orders false. not_a_forecast. Soft PASS N/A ≠ arm.
Do NOT edit phase1/120–126 or config/default.yaml.
...
PHASE1 = 127
SOURCE = "public_md_scalp_structure_bos_127"
ID_FAMILY = "public_md_v1_structure_bos_15m_rsi3m_1m_long_only_ft_rsi_bands"
PARENT_PHASE1 = 126
...
def _assert_candidate_id_ok(cid: str) -> None:
    ...
    if not cid.startswith("public_md_v1_structure_bos_15m_rsi3m_1m_long_only_ft_rsi_bands_"):
        raise ReplayError(f"candidate id must be structure-bos 127 scoped: {cid}")
...
def walk_structure_bos_127(
    ...
    *,
    ...
    """Long-only BOS walker: SL / 1.5R TP / opp-BOS / RSI≥67 / time-stop.

    Fills at OPEN from previous bar close signal (confirm_closed_only).
    Reuses #125 walk mechanics; adds rsi_exit on first in-force 3m RSI ≥ 67.
    No shorts. One position, full sleeve, no martingale, no leverage.
    """
    bars = bars_1m
    if not bars:
    ...
    def _count_exit(reason: str) -> None:
        nonlocal n_tp_exits, n_sl_exits, n_opp_bos_exits, n_rsi_exits, n_time_exits
        if reason == "tp":
            n_tp_exits += 1
        elif reason == "sl":
            n_sl_exits += 1
    ...
    for i, bar in enumerate(bars):
        ...
        if pending is not None and in_trade:
            ...
            if pending == LONG and qty == 0.0:
                ...
                # Long-only: should never arm SHORT; refuse fill
        ...
        if qty != 0.0 and entry_px > 0.0 and sl_px > 0.0:
            ...
            if qty > 0.0:
                ...
                # RSI exit: first in-force 3m RSI ≥ 67 (honesty: no wait for 75)
                if not want_exit and rsi_exit_triggered(signals.rsi[i]):
                    # Guard: rsi_exit_min lock
                    if float(signals.rsi[i]) >= float(rsi_exit_min):  # type: ignore[arg-type]
                        want_exit, exit_reason = True, "rsi_exit"
            if not want_exit and held_bars >= int(time_stop_bars):
        ...
        if want_exit and have != FLAT:
            ...
            if es:
                # long_only: ignore shorts (should be all False from precompute)
                es = False
...
def score_cell(
    ...
    *,
    ...
    return {
        "ok": True,
        "status": "MEASURED",
        "family_key": "structure_bos_127_ft_rsi_bands",
        "family_label": (
            "15m structure + 3m RSI band 20-30 + 1m BOS follow-through long-only"
        ),
        "inst_id": inst_id,
        "window_key": window_key,
        ...
        "n_rsi_exits": walk.get("n_rsi_exits"),
...
def _vs_126_rows(
    cells_127: list[dict[str, Any]], parent: dict[str, Any] | None
) -> list[dict[str, Any]]:
    parent_cells = {
    ...
    for c in cells_127:
        ...
        n127 = int(c.get("n_trades") or 0)
        n126 = int(p.get("n_trades") or 0) if p else None
        rows.append(
            {
                ...
                "n_trades_127": n127,
                "n_trades_126": n126,
                "n_trades_drop": (n126 - n127) if n126 is not None else None,
                "n_long_entries_127": c.get("n_long_entries"),
                "n_short_entries_127": c.get("n_short_entries"),
                "n_long_entries_126": p.get("n_long_entries"),
                "n_short_entries_126": p.get("n_short_entries"),
                "exp_127": c.get("expectancy_completed_eur"),
                "exp_126": p.get("expectancy_completed_eur"),
                "terminal_127": c.get("terminal_liquidation_net_eur"),
                "terminal_126": p.get("terminal_liquidation_net_eur"),
                "bh_net": c.get("bh_net_return_eur"),
                "exit_mix_127": {
                    "tp": c.get("n_tp_exits"),
                    "sl": c.get("n_sl_exits"),
                    "opp_bos": c.get("n_opp_bos_exits"),
                    "rsi_exit": c.get("n_rsi_exits"),
                ...
                "pair_pass_full_127": c.get("pair_pass_full"),
...
def run_structure_bos_127_score(
    cfg: Any,
    *,
    data_dir: Path,
    ...
    _strat = StructureBos127V1(StructureBos127Params())
    http = client
    ...
    vs_126 = _vs_126_rows(cells, parent)
    ...
    return {
        ...
        "id_family": ID_FAMILY,
        "family": FAMILY,
        "strategy": "atlas.strategy.scalp_structure_bos_127",
        "reuses_strategy_helpers": "atlas.strategy.scalp_structure_bos_125",
        "reuses_cache": CACHE_DIR_NAME,
        "walker": "walk_structure_bos_127",
        "long_only": True,
        ...
        "locks": {
            ...
            "delta_vs_126": (
                "bos_follow_through + RSI entry [20,30] + RSI exit >=67"
            ),
        },
        "pass_rule": (
        ...
        "do_not_edit_phase1": ["120", "121", "122", "123", "124", "125", "126"],
        "s1_transplant": False,
        "mid_71_transplant": False,
        "scores_126_transplant": False,
        "no_60_parallel": True,
