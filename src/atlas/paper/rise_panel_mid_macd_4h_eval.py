"""rise_panel_v1 Mid #68 — DOGE 4H MACD(12,26,9) €40 vs NEW Mid Breakout #65 baseline.
...
Canonical MACD long/flat: long on MACD×signal cross-up; flat on cross-down.
No short. No RSI. No daily-bull. No EMA period grind.
...
SOURCE = "rise_panel_v1_mid_macd_68"
WARMUP_PAD_4H_DAYS = 20  # MACD(12,26,9) needs ~34 bars (~6d) + slack
...
CORE_MID_BOOK_ID = "rise_panel_v1_core_mid_book_180_mid_macd12269_4h"
...
def run_mid_macd_4h(
    ...
    *,
    ...
    """Mid MACD(12,26,9) 4H long/flat on SAME locked 7 — candidate #68."""

    def factory() -> MidDogeMacd4hV1:
        return MidDogeMacd4hV1()

    out = _run_panel(
        cfg,
        data_dir=data_dir,
        strategy_factory=factory,
        arm="mid_macd12269_4h",
        candidate_id=MID_IMPROVE_ID,
        strategy_label=f"macd{FAST}_{SLOW}_{SIGNAL}_cross_long_flat",
        family=FAMILY,
        pause_s=pause_s,
        rest_base=rest_base,
        apply_soft_promote=True,
    ...
    out["family"] = FAMILY
    out["reuse_note"] = (
        "MacdTrendV1 via mid_doge_macd_4h; Mid #68 locks MACD(12,26,9) cross "
        "long/flat on 4H (canonical paper rule; no prior Mid MACD in-repo)."
    )
    return out


def deltas_vs_mid_baseline(baseline: dict[str, Any], improve: dict[str, Any]) -> dict[str, Any]:
    """Panel metric deltas: MACD Mid − Mid Breakout #65 baseline (NOT vs Core)."""
...
def deltas_vs_ema_archive(improve: dict[str, Any]) -> dict[str, Any]:
    """Optional honesty: MACD Mid − archived EMA12/30 Mid snapshot."""
    i = improve.get("summary") or {}
...
def run_core_mid_book(
    ...
    *,
    ...
    """Core €140 1D EMA + this Mid €40 MACD(12,26,9) — book €180, NO Scalp sleeve.
...
def render_results_markdown(
    ...
    *,
    ...
    """Full phase1/68 doc with lock + scored results (+ Core+Mid book if promote)."""
    if deltas is None:
    ...
    lines.append(
        ...
        "(./65b-mid-breakout-promote.md). EMA Mid 12/30 = archive only. "
        "Canonical **MACD(12,26,9)** cross long/flat. **No** RSI. **No** daily-bull. **No** EMA grind."
    )
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    lines.append(
        ...
        "— **not** current Mid baseline (archive EMA12/30; this trial is MACD)."
    )
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "**ONE family only — NOT grinding MACD periods / TF / costs. No RSI. No daily-bull. "
        "Do not re-grind Breakout / RSI MR / Donchian / EMA12/21 / EMA12/30.**"
    )
    lines.append("")
    ...
    lines.append(
        f"**Canonical MACD (paper rule):** MACD(**{FAST}**,**{SLOW}**,**{SIGNAL}**) cross long/flat. "
        "Decision bar **4H**. **No** RSI. **No** daily-bull."
    )
    lines.append("")
    ...
    lines.append(
        "- **No** RSI filter. **No** daily-bull regime gate "
    ...
    lines.append(
        ...
        "No prior Mid/Scalp MACD module in-repo — this trial locks canonical "
        "**MACD(12,26,9)** cross long/flat onto Mid €40 / locked rise panel — "
        "testing MACD vs Breakout without inventing RSI / Breakout / EMA knobs."
    )
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    lines.append("")
    lines.append("- Script: `scripts/run_rise_panel_mid_macd_4h_eval.py`")
    lines.append("- Module: `atlas.paper.rise_panel_mid_macd_4h_eval`")
    lines.append(
        "- Strategy: `atlas.strategy.mid_doge_macd_4h` "
        "(MacdTrendV1 wrapper; MACD 12/26/9 locked)"
    )
    lines.append(
        "- Reuse: `walk_long_flat`, `soft_promote_score`, locked `RISE_PANEL_V1` windows; "
        "Core+Mid book €180 on PASS+better only (no Scalp)"
    )
    lines.append("- Unit tests: `tests/unit/test_rise_panel_mid_macd_4h.py`")
    lines.append("")
    lines.append("---")
    lines.append("")
    lines.append("## D. Results — Mid MACD(12,26,9) 4H €40 on same 7")
    lines.append("")
    lines.append(f"**mid_improve_id:** `{MID_IMPROVE_ID}`")
    lines.append("")
    ...
    lines.append("")
    lines.append("**Panel summary (Mid MACD 4H):**")
    lines.append(
    ...
    lines.append("")
    lines.append(
        f"### Soft promote (Mid MACD): **{soft.get('verdict', '—')}** (`{SOFT_PROMOTE_GATE}`)"
    )
    lines.append("")
    lines.append(
    ...
    lines.append("")
    lines.append("### Honesty deltas vs Mid Breakout #65 baseline (MACD − Breakout)")
    lines.append("")
    lines.append(
    ...
    lines.append("")
    lines.append("### Honesty deltas vs EMA12/30 Mid archive (optional; MACD − EMA12/30)")
    lines.append("")
    lines.append(
    ...
    if promote_as_better and core_mid_book is not None:
        ...
        lines.append(f"- **mid_id:** `{MID_IMPROVE_ID}` (4H MACD 12/26/9)")
        ...
        lines.append("")
        lines.append("| Sleeve | #65 Core+Mid (Breakout Mid) | #68 Core+Mid (this Mid) | Δ |")
        lines.append("|--------|----------------------------:|------------------------:|--:|")
        lines.append(
        ...
        lines.append("")
        lines.append("Reports: `data/reports/rise_panel_v1_core_mid_book_mid_macd_68.json`")
    ...
    lines.append("")
    lines.append("- Not a MACD period / TF / cost grind (12/26/9 locked).")
    lines.append("- Not EMA12/21 (#67) or EMA12/30 archive.")
    lines.append("- Not RSI MR (#66) or RSI filter.")
    lines.append("- Not BreakoutV1 (#65) or Donchian 20/10 (#64).")
    lines.append("- Not Scalp (this is Mid 4H; Scalp = Kaje manual).")
