"""rise_panel_v1 Mid #67 — DOGE 4H EMA12/21 €40 vs NEW Mid Breakout #65 baseline.
...
Plain EMA long/flat periods 12/21 (NOT 12/30 archive). No RSI. No daily-bull.
...
SOURCE = "rise_panel_v1_mid_ema1221_67"
...
CORE_MID_BOOK_ID = "rise_panel_v1_core_mid_book_180_mid_ema12_21_4h"
...
def run_mid_ema1221_4h(
    ...
    *,
    ...
    """Mid EMA12/21 4H long/flat on SAME locked 7 — candidate #67."""

    def factory() -> MidDogeEma1221V1:
        return MidDogeEma1221V1()
    ...
    out["family"] = FAMILY
    out["reuse_note"] = (
        "EmaTrendV1 via mid_doge_ema1221_4h; Mid #67 locks plain EMA12/21 long/flat "
        "on 4H (≠ Mid EMA12/30 archive; ≠ Scalp #63 1H)."
    )
    return out


def deltas_vs_mid_baseline(baseline: dict[str, Any], improve: dict[str, Any]) -> dict[str, Any]:
    """Panel metric deltas: EMA12/21 Mid − Mid Breakout #65 baseline (NOT vs Core)."""
...
def deltas_vs_ema_archive(improve: dict[str, Any]) -> dict[str, Any]:
    """Optional honesty: EMA12/21 Mid − archived EMA12/30 Mid snapshot."""
    i = improve.get("summary") or {}
...
def run_core_mid_book(
    ...
    *,
    ...
    """Core €140 1D EMA + this Mid €40 EMA12/21 — book €180, NO Scalp sleeve.
...
def render_results_markdown(
    ...
    *,
    ...
    """Full phase1/67 doc with lock + scored results (+ Core+Mid book if promote)."""
    if deltas is None:
    ...
    lines.append(
        ...
        "Plain **EMA12/21** long/flat (NOT 12/30). **No** RSI. **No** daily-bull."
    )
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    lines.append(
        ...
        "Do not re-grind Breakout / RSI MR / Donchian / EMA12/30.**"
    )
    lines.append("")
    ...
    lines.append(
        f"**Canonical EMA (plain Mid 4H style):** EMA(**{FAST}**/**{SLOW}**) long/flat. "
    ...
    lines.append(f"- EMA: fast **{FAST}** / slow **{SLOW}** (NOT 12/30).")
    lines.append(
    ...
    lines.append(
        ...
        "(distinct from Breakout #65, RSI MR #66, Donchian #64, EMA12/30 archive)."
    )
    ...
    lines.append(
        ...
        "This trial ports plain **EMA12/21** (like Scalp #63 periods, Mid 4H TF) onto Mid €40 / "
        "locked rise panel — testing faster EMA twin vs Breakout without inventing RSI / "
        "Breakout knobs. Distinct from archived EMA12/30 Mid."
    )
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    lines.append("")
    lines.append("- Script: `scripts/run_rise_panel_mid_ema1221_4h_eval.py`")
    lines.append("- Module: `atlas.paper.rise_panel_mid_ema1221_4h_eval`")
    lines.append(
        "- Strategy: `atlas.strategy.mid_doge_ema1221_4h` "
        "(EmaTrendV1 wrapper; periods 12/21 locked)"
    )
    lines.append(
        "- Reuse: `walk_long_flat`, `soft_promote_score`, locked `RISE_PANEL_V1` windows; "
        "Core+Mid book €180 on PASS+better only (no Scalp)"
    )
    lines.append("- Unit tests: `tests/unit/test_rise_panel_mid_ema1221_4h.py`")
    lines.append("")
    lines.append("---")
    lines.append("")
    lines.append("## D. Results — Mid EMA12/21 4H €40 on same 7")
    lines.append("")
    lines.append(f"**mid_improve_id:** `{MID_IMPROVE_ID}`")
    lines.append("")
    ...
    lines.append("")
    lines.append("**Panel summary (Mid EMA12/21 4H):**")
    lines.append(
    ...
    lines.append("")
    lines.append(
        f"### Soft promote (Mid EMA12/21): **{soft.get('verdict', '—')}** (`{SOFT_PROMOTE_GATE}`)"
    )
    lines.append("")
    lines.append(
    ...
    lines.append("")
    lines.append("### Honesty deltas vs Mid Breakout #65 baseline (EMA12/21 − Breakout)")
    lines.append("")
    lines.append(
    ...
    lines.append("")
    lines.append("### Honesty deltas vs EMA12/30 Mid archive (optional; EMA12/21 − EMA12/30)")
    lines.append("")
    lines.append(
    ...
    if promote_as_better and core_mid_book is not None:
        ...
        lines.append(f"- **mid_id:** `{MID_IMPROVE_ID}` (4H EMA12/21)")
        ...
        lines.append("")
        lines.append("| Sleeve | #65 Core+Mid (Breakout Mid) | #67 Core+Mid (this Mid) | Δ |")
        lines.append("|--------|----------------------------:|------------------------:|--:|")
        lines.append(
        ...
        lines.append("")
        lines.append("Reports: `data/reports/rise_panel_v1_core_mid_book_mid_ema1221_67.json`")
    ...
    lines.append("")
    lines.append("- Not an EMA period / TF / cost grind (12/21 locked; not 12/30).")
    lines.append("- Not RSI MR (#66) or RSI filter.")
    lines.append("- Not BreakoutV1 (#65) or Donchian 20/10 (#64).")
    lines.append("- Not Mid EMA12/30 archive (different periods).")
    lines.append("- Not Scalp #63 1H EMA12/21 (this is Mid 4H).")
