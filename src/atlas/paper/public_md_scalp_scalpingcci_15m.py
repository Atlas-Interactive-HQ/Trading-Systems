"""Public-MD Scalp #124 — guibvieira ScalpingCCI 15m native scores.

GPL-3.0 source cited; Atlas REIMPLEMENTATION (not a verbatim copy of ScalpingCCI.py).
BTC-USDT / ETH-USDT / DOGE-USDT. Windows Jul2020→Jan2021 (+ SUB A/B).
€20 sleeve, 5+5 bps, next-open fills, source ROI ladder + SL −0.04 locked exits.
...
Do not edit phase1/120/121/122/123. No PEPE. No Mid #71. No S1 transplant.
No 1H adaptation. No 121 grind. No 123 param-rescue.
"""
...
PHASE1 = 124
SOURCE = "public_md_scalp_scalpingcci_15m_124"
ID_FAMILY = "public_md_v1_guibvieira_scalping_cci_15m_long_flat"
...
CACHE_DIR_NAME = "public_md_124_cache"
...
# ~17664 FULL 15m bars expected; fail closed well below
...
def _assert_candidate_id_ok(cid: str) -> None:
    ...
    if not cid.startswith("public_md_v1_guibvieira_scalping_cci_15m_"):
        raise ReplayError(f"candidate id must be scalpingcci 15m scoped: {cid}")
...
def walk_long_flat_roi_ladder_sl(
    bars: list[Bar],
    *,
    ...
    """Causal long/flat walk with indicator signals + ROI ladder + SL locked exits.

    Fills at OPEN from the previous bar's close signal.
    ROI: closed-bar close >= entry_fill * (1 + roi_threshold(minutes_held)).
    SL: close <= entry_fill * (1 - sl).
    ...
    for i, bar in enumerate(bars):
        ...
        if pending is not None and in_trade:
            if pending == LONG and qty == 0.0:
                ...
                entry_px = px
                entry_fee = fee
                entry_fill_ts_ms = int(bar.ts_open_ms)
                n_entries += 1
        ...
        if qty > 0.0 and entry_px > 0.0:
            minutes_held = max(
                0.0, (float(bar.ts_close_ms) - float(entry_fill_ts_ms)) / 60_000.0
            )
            roi_frac = roi_threshold_for_minutes(minutes_held)
    ...
    out = {
        ...
        "roi_ladder": [{"minutes": m, "roi": r} for m, r in ROI_LADDER],
        "sl_frac": float(sl_frac),
        "not_a_forecast": True,
        "place_orders": False,
    }
...
def cache_path_for(results_dir: Path, inst_id: str) -> Path:
    return Path(results_dir) / CACHE_DIR_NAME / f"{inst_id}_15m.jsonl"


def fetch_usdt_15m_series(
    client: httpx.Client,
    inst_id: str,
    *,
    ...
    """Fetch closed 15m bars covering warmup 2020-06-01 → 2021-01-01 exclusive."""
    ...
    if not bars:
        raise ReplayError(f"empty 15m series for {inst_id} (fail closed)")
...
def score_cell(
    ...
    *,
    ...
    return {
        "ok": True,
        "status": "MEASURED",
        "family_key": "scalping_cci_15m",
        "family_label": "guibvieira ScalpingCCI 15m long/flat (GPL reimpl)",
        "inst_id": inst_id,
        "window_key": window_key,
        ...
        "candidate_id": cid,
        "id_family": ID_FAMILY,
        "mechanism": "atlas.strategy.scalping_cci_15m",
        "bar": BAR,
...
def run_scalpingcci_15m_score(
    ...
    *,
    ...
    """Fetch three USDT 15m series (cache under results/) and score windows."""
    ...
    _strat = ScalpingCci15mV1(ScalpingCci15mParams())
    ...
    try:
        for inst in USDT_INSTS:
            ...
            try:
                bars: list[Bar] = []
                if use_cache and cpath.is_file() and cpath.stat().st_size > 100_000:
    ...
    return {
        ...
        "source": SOURCE,
        "stance": (
            "Research / public-MD scores of GPL-cited ScalpingCCI 15m reimpl. "
        ...
        "claimed_window_in_source": "UNVERIFIED (no timerange in ScalpingCCI.py)",
        ...
        "do_not_edit_phase1": ["120", "121", "122", "123"],
        "s1_transplant": False,
        "mid_71_transplant": False,
