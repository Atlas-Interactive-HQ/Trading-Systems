"""Mid daily EMA pullback long eval — low-freq sleeve alongside Core spot EMA.
...
MID_RISK_FRAC = 0.015
LOW_FREQ_MEDIAN_CAP = 15  # sanity; FLAG if median trades/window > this
...
def run_mid_slice(
    *,
    ...
    # Inclusive of last trade bar close for fill/exit management past signal day.
    trade_end = trade_bars[-1].ts_close_ms + 1
    # Bars from earliest pad through end of slice (causal indicators + exits).
...
def evaluate_window(
    ...
    *,
    ...
    return {
        ...
        "scalp": {
            "out_of_trial": True,
            "note": "Scalp halted for this trial — Mid daily only",
        },
...
def aggregate_set(window_results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    # low_freq is FLAG only — does not flip PASS→FAIL
    overall = bool(mid_ok)
    return {
...
def run_set(
    ...
    *,
    ...
    return {
        ...
        "rules": {
            "regime": "signal-day closed daily EMA12 > EMA30 on DOGE-USDT; else no new entries; no shorts",
            "mid": (
                f"daily pullback long 1D; low≤EMA12 or prior close<EMA12 then close>EMA12 & >EMA30; "
...
def _render_window(wr: dict[str, Any]) -> list[str]:
    ...
    mid = wr.get("mid") or {}
    lines.append(
        f"**Mid (daily pullback atr_stop={MID_ATR_STOP_MULT} TP={MID_TP_R}R "
...
def render_markdown(bundle_a: dict[str, Any], bundle_b: dict[str, Any] | None) -> str:
    lines: list[str] = []
    lines.append("# 36 — Mid daily EMA pullback long (low-freq; Scalp OUT)")
    lines.append("")
    ...
    lines.append("**Family:** Mid daily pullback long alongside Core EMA regime. Scalp halted. Not BreakoutV1.")
    lines.append("")
    ...
    lines.append("")
    lines.append("- NEW entries only when **signal-day closed daily EMA12 > EMA30** on DOGE-USDT.")
    ...
    lines.append("")
    lines.append("### Mid — daily pullback long (spot 1D, €40)")
    lines.append("")
    ...
    lines.append(f"- Low-freq sanity: median full-window trades ≤ {LOW_FREQ_MEDIAN_CAP} (FLAG if not)")
    lines.append("")
