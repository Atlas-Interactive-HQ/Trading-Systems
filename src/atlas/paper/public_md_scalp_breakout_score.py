"""Public-MD Scalp scores (phase1/118) — BreakoutV1 1H long/flat on spot memes.

LOCKED first-score hyp for BreakoutV1 family. Research only. not_a_forecast.
Never places orders. Does NOT mutate config/default.yaml.
Soft PASS ≠ arm (Soft PASS N/A — not an arm).
NOT Scalp S1 Dual Thrust/RVOL. NOT an EMA12/21 grind (#117). NOT #62 panel €.
Reuses BreakoutV1 long/flat *mechanism* from atlas.strategy.scalp_doge_breakout_1h
(Scalp #62 plumbing) with NEW per-inst candidate ids.
Rule: closed close > prior Donchian-16 high AND ATR/close >= 0.001 → long;
      closed close < prior Donchian-16 low → flat. ATR SMA 14. oneh_filter off.
      Never short. confirm_closed_only. Channel exit only in walk_long_flat
      (no ATR-stop in this harness — same as #62 comment).
...
PHASE1 = 118
SOURCE = "public_md_scalp_breakout_score_118"
ID_FAMILY = "public_md_v1_breakout_v1_1h_long_flat"
BAR = "1H"
...
MIN_TRADE_BARS = LOOKBACK + ATR_PERIOD + 2
...
# Forbidden #62 / #117 id families — new per-inst ids only.
...
# Same windows as #117. end is resolved at run time via last_closed_1h_end_exclusive_ms().
...
# Prior measured n (cite only as prior probe from #117; run reports real measured n).
PRIOR_MEASURED_N_NOTE: dict[str, str] = {
    "PUMP-USDC": "n≈4746 measured 2026-09-12 (#117 probe; history-candles)",
    "TRUMP-USDC": "n≈4818 measured 2026-09-12 (#117 probe; history-candles)",
    "WIF-USDC": "n=6000 at 20-page cap 2026-09-12; do not invent older",
}


def candidate_id_for(inst_id: str) -> str:
    """New per-inst candidate id. Never S1 / never #62 / never #117 EMA ids."""
...
def make_strategy() -> ScalpDogeBreakout1hV1:
    """BreakoutV1 confirm_closed_only long/flat. Never short. No DT / RVOL / EMA grind."""
...
def score_inst(
    ...
    *,
    ...
    """Walk locked BreakoutV1 long/flat; report completed expectancy + accounting_v2 terminal."""
    ...
    return {
        ...
        "min_atr_frac": MIN_ATR_FRAC,
        "oneh_filter": "off",
        "atr_stop_in_harness": False,
        "channel_exit_only": True,
        "confirm_closed_only": True,
        "never_short": True,
        "rsi": False,
        "daily_bull": False,
...
def run_breakout_score(
    ...
    *,
    ...
    """Download three spot 1H series and score locked BreakoutV1. Fail closed on error."""
    ...
    errors: list[str] = []
    cache_dir = Path(data_dir) / "paper" / "candles" / "public_md_118"
    ...
    return {
        ...
        "rule": (
            ...
            "(no ATR-stop in walk_long_flat harness)"
        ),
        "bar": BAR,
        "lookback": LOOKBACK,
        "atr_period": ATR_PERIOD,
        ...
        "mechanism_reuse": "atlas.strategy.scalp_doge_breakout_1h (#62 plumbing)",
        "not_ema_family": True,
        "not_s1": True,
        "not_62_panel_eur": True,
        "soft_pass_applied_as_arm": False,
        "soft_pass_status": "N/A_not_an_arm",
