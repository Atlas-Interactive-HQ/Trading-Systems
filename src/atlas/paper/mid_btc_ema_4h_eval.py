"""Mid BTC-USDT 4H EMA12/30 long-only — Core-style RETURN gate at Mid €40.

LOCKED #49. SAME EMA12/30 long/flat rule as mid_doge_ema_4h (#45) /
EmaTrendV1 / walk_long_flat, on **BTC-USDT 4H** (bull TF Mid; ≠ DOGE #45 asset).
...
prior Mid gate despite full+ bull windows.

Research only. not_a_forecast. Does NOT mutate config/default.yaml. Never places
orders. Never invents metrics. Scalp OUT. Set A AND set B each must PASS.
...
from atlas.strategy.mid_btc_ema_4h import BAR, FAMILY, FAST, SLOW

SOURCE = "mid_btc_ema_4h"
SPOT_MD = "BTC-USDT"
# ≥ EMA30 warmup on 4H (30×4H ≈ 5d) + buffer
WARMUP_PAD_DAYS = 10
...
SAME_RULE_AS = "mid_doge_ema_4h (#45) / EmaTrendV1 — BTC asset diversify ≠ DOGE twin; TF Mid 4H"


def resolve_windows(set_id: str, *, data_dir: Path, rest_base: str, pause_s: float) -> list[CascadeWindow]:
    """Same A/B calendars as recent Mid trials; probe 4H MD; label fallbacks."""
    if set_id == "A":
...
def _holdout_window(window: CascadeWindow, hold_bars: list[Bar]) -> CascadeWindow:
    if not hold_bars:
        raise ReplayError("empty holdout for mid btc ema 4h")
...
def run_mid_ema_slice(
    *,
    ...
    """Full-sleeve EMA12/30 long/flat on Mid equity (BTC-USDT 4H)."""
...
def evaluate_window(
    ...
    *,
    ...
    trade = [b for b in bars_4h if window.start_ms <= b.ts_open_ms < window.end_ms_exclusive]
    if len(trade) < 12:
        raise ReplayError(f"insufficient 4H trade bars for {window.id}")
    ...
    mid_full = run_mid_ema_slice(
        ...
        fee_rate=fee_rate,
        slippage_bps=slip,
        label=f"mid-ema4h-full-{window.id}",
    )
    mid_hold = None
    if hold:
        ...
        mid_hold = run_mid_ema_slice(
            ...
            fee_rate=fee_rate,
            slippage_bps=slip,
            label=f"mid-ema4h-hold-{window.id}",
        )
    ...
    return {
        ...
        "n_bars_4h_pad": len(bars_4h),
        "n_bars_4h_trade": len(trade),
        "n_bars_daily_pad": len(daily),
        "split": {
            "frac_in_sample": SPLIT_FRAC,
            "n_bars_in_sample": len(ins),
            "n_bars_holdout": len(hold),
            "rule": "first 70% of 4H trade bars by time, last 30% holdout; cut never searched",
        },
        ...
        "mid": {
            ...
            "differs_from_holdout_exp_gate": True,
            "differs_reason": DIFFERS_REASON,
            "tf_note": "bull TF Mid BTC 4H ≠ DOGE 4H twin #45",
        },
        "scalp": {
            "out_of_trial": True,
            "note": "Scalp halted for this trial — Mid BTC EMA 4H Core-style return gate only",
        },
...
def aggregate_set(window_results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "core": {
            "scored": False,
            "note": "informational only — EMA12/30 long sleeve €140 / BH on BTC-USDT 1D; not in overall gate",
        },
    ...
    return {
        ...
        "rules": {
            "mid": (
                ...
                f"full-sleeve long/flat on Mid €{MID_START_EUR:.0f} BTC-USDT {BAR}; "
                f"DD ≤ BH×{BH_DD_MULT:g} else ≤€{MID_DD_ABS_CAP_EUR}"
            ),
            "scalp": "OUT of this trial (halt)",
            "core": "informational EMA12/30 long / BH on BTC-USDT €140 1D — not scored for overall",
...
def _render_window(wr: dict[str, Any]) -> list[str]:
    ...
    if wr.get("error"):
        ...
        return lines
    lines.append(
        f"MD bars: 4H(pad)={wr.get('n_bars_4h_pad')} trade={wr.get('n_bars_4h_trade')} "
    ...
    lines.append("**Core (informational — EMA12/30 long / BH on BTC-USDT 1D €140)**")
    lines.append("")
    ...
    mid = wr.get("mid") or {}
    lines.append(
        f"**Mid (BTC EMA{FAST}/{SLOW} long/flat full-sleeve €{MID_START_EUR:.0f} on **{BAR}**; "
...
def render_set_markdown(bundle: dict[str, Any]) -> str:
    ...
    lines.append(f"# Mid #49 BTC EMA 4H — set {sid}")
    lines.append("")
...
def render_markdown(bundle_a: dict[str, Any], bundle_b: dict[str, Any] | None) -> str:
    lines: list[str] = []
    lines.append("# 49 — Mid BTC EMA12/30 on **4H** Core-style RETURN gate (Mid sleeve; Scalp OUT)")
    lines.append("")
    ...
    lines.append(
        "**Family:** Mid **EMA12/30 long/flat** on **BTC-USDT 4H** — SAME Core rule "
        f"(`EmaTrendV1` / `walk_long_flat` / #41/#45 spirit) at Mid €40. **Bull TF Mid on BTC** (≠ DOGE #45; asset diversify). "
    ...
    lines.append("")
    lines.append("### Mid — EMA12/30 long/flat (spot BTC-USDT **4H**, €40)")
    lines.append("")
    ...
    lines.append(f"- Bar: **{BAR}** (bull TF Mid on BTC; ≠ DOGE 4H twin #45).")
    lines.append("- Low n_trades: **OK** (document n_trades / TIM / expectancy; not a FAIL gate).")
    lines.append("- Windows: same A/B calendars as recent Mid trials mapped to BTC 4H bars; MD fallbacks labeled.")
    lines.append("")
    lines.append("### Core (informational only)")
    lines.append("")
    lines.append("- Same rule on BTC-USDT €140 **1D** reported as context only.")
    ...
    lines.append(
        ...
        "Mirrors scoring spirit of mid_doge_ema_4h (#45) / mid_doge_ema_coregate (#41) adapted to this BTC 4H trial. "
