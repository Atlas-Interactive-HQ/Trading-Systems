"""Mid BTC-USDT 1D Donchian long-only + EMA regime eval.

NEW entry family vs Mid daily pullback (#38 FAIL). Research only. not_a_forecast.
Does NOT mutate config/default.yaml. Never places orders. Never invents metrics.
No ATR stop this trial (avoid dual knobs). Time stop 15d. Scalp OUT.
Core EMA/BH informational only — not in overall gate.
"""
...
SOURCE = "mid_btc_donchian_ema"
SPOT_MD = "BTC-USDT"
WARMUP_DAILY = 40

MID_TIME_STOP_BARS = TIME_STOP_BARS  # 15 trading days
...
def mid_strategy() -> MidBtcDonchianEmaV1:
    return MidBtcDonchianEmaV1(MidBtcDonchianEmaParams(sleeve="mid"))
...
def run_mid_slice(
    *,
    ...
    """Causal Mid walk: pad history for EMA/Donchian; NEW entries only inside trade_bars span."""
    if not trade_bars:
        return {
            "ok": False,
            "fail_closed": True,
    ...
    eng = PaperEngine(
        ...
        gated,
        journal=NullJournal(),
        run_id=f"mid-btc-donchian-ema-{label}",
        data_dir="data",
    )
...
def score_mid(full: dict[str, Any], hold: dict[str, Any] | None, *, dd_cap_eur: float) -> dict[str, Any]:
    """Holdout expectancy must be > 0 (stricter Mid BTC gate)."""
    ...
    *,
    ...
    return {
        ...
        "mid": {
            ...
            "family": "donchian_breakout_ema_regime",
        },
        "scalp": {
            "out_of_trial": True,
            "note": "Scalp halted for this trial — Mid BTC Donchian+EMA only",
        },
...
def run_set(
    ...
    *,
    ...
    return {
        ...
        "bar": "1D",
        "family": "donchian_breakout_ema_regime",
        "prior_family_fail": "mid_btc_daily_pullback (#38)",
        ...
        "rules": {
            ...
            "mid": (
                f"Donchian long-only 1D BTC-USDT; entry close > prior {ENTRY_LOOKBACK}-day high; "
                f"exit close < prior {EXIT_LOOKBACK}-day low OR regime flat OR time {MID_TIME_STOP_BARS}d; "
                f"NO ATR stop; structural sizing stop = prior {EXIT_LOOKBACK}-low at entry; "
            ...
            "holdout": "holdout expectancy > 0 (not merely not-worse-than-full)",
            "low_freq": f"median full n_trades ≤ {LOW_FREQ_MEDIAN_CAP} is a PASS gate",
            "no_atr_stop": True,
            "new_family_vs_pullback": True,
        },
        "results": results,
        "aggregate": agg,
        "errors": errors,
...
def _render_window(wr: dict[str, Any]) -> list[str]:
    ...
    mid = wr.get("mid") or {}
    lines.append(
        f"**Mid (BTC Donchian {ENTRY_LOOKBACK}/{EXIT_LOOKBACK} + EMA regime; "
        f"NO ATR; time={MID_TIME_STOP_BARS}d DD_cap=€{MID_DD_CAP_EUR})**"
    )
    lines.append("")
...
def render_markdown(bundle_a: dict[str, Any], bundle_b: dict[str, Any] | None) -> str:
    lines: list[str] = []
    lines.append("# 39 — Mid BTC Donchian + EMA regime long (new family; Scalp OUT)")
    lines.append("")
    ...
    lines.append(
        "**Family:** Mid **Donchian breakout** long on **BTC-USDT 1D** + EMA12/30 regime. "
        "**Different family** from Mid daily pullback (#38 FAIL). No ATR stop this trial. Scalp halted."
    )
    lines.append("")
    ...
    lines.append("")
    lines.append("### Mid — Donchian breakout long (spot BTC-USDT 1D, €40)")
    lines.append("")
    lines.append(f"- Entry: close > prior {ENTRY_LOOKBACK}-day high (Donchian breakout up). Long only.")
    lines.append(
        f"- Exit (NO ATR stop): close < prior {EXIT_LOOKBACK}-day low OR EMA12 ≤ EMA30 OR time stop "
        f"{MID_TIME_STOP_BARS} trading days."
    )
    lines.append(
        f"- Structural stop for 1.5% risk sizing / engine protective floor = prior {EXIT_LOOKBACK}-day "
    ...
    lines.append("")
    lines.append("### PASS gates (Mid only)")
    lines.append("")
    lines.append("- Expectancy after costs > 0 on ≥2 of 3 FULL windows")
    lines.append("- On each such window: holdout n_trades≥1 AND holdout expectancy **> 0**")
    ...
    lines.append("")
    lines.append("- Do **not** add ATR stop, change Donchian lookbacks, EMA periods, time stop, risk %, or bar size to chase PASS.")
    ...
    lines.append("- This is a **new family** vs pullback — do not blend pullback reclaim rules back in.")
    lines.append("")
