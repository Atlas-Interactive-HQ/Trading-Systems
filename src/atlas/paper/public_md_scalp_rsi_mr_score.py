"""Public-MD Scalp scores (phase1/119) — RSI14 mean-reversion 1H long/flat on spot memes.

LOCKED complementary SCORE hyp (mean-reversion vs trend board). Research only.
not_a_forecast. Never places orders. Does NOT mutate config/default.yaml.
Soft PASS ≠ arm (Soft PASS N/A — not an arm).
NOT Scalp S1 Dual Thrust/RVOL. NOT an EMA12/21 grind (#117). NOT BreakoutV1 (#118).
NOT a copy of #59 panel € / panel ids.
Reuses RSI(14) MR long/flat *mechanism* from atlas.strategy.scalp_doge_rsi_mr_1h
(Scalp #59 plumbing) with NEW per-inst candidate ids.
Rule: RSI(14) Wilder; entry = cross-up from ≤30 (prev ≤30 and curr >30);
      exit = closed RSI ≥70 → flat. No EMA filter. Never short. confirm_closed_only.
Data: OKX EEA public history-candles (+ candles if needed).
No demo OMS. No invented bars. Spot-primary: PUMP / TRUMP / WIF USDC.
Team-fit: complement to Mid #71 / Breakout #118 — different payoff family.
"""
...
PHASE1 = 119
SOURCE = "public_md_scalp_rsi_mr_score_119"
ID_FAMILY = "public_md_v1_rsi14_mr_1h_long_flat"
BAR = "1H"
...
# Forbidden #59 / #117 / #118 id families — new per-inst ids only.
FORBIDDEN_ID_SUBSTRINGS: tuple[str, ...] = (
    "dual_thrust",
    "rvol",
    "rise_panel",
    "ema12_21",
...
# Same windows as #117/#118. end is resolved at run time via last_closed_1h_end_exclusive_ms().
...
# Prior measured n (cite only as prior probe from #117/#118; run reports real measured n).
PRIOR_MEASURED_N_NOTE: dict[str, str] = {
    "PUMP-USDC": "n≈4746 measured 2026-09-12 (#117/#118 probe; history-candles)",
    "TRUMP-USDC": "n≈4818 measured 2026-09-12 (#117/#118 probe; history-candles)",
    "WIF-USDC": "n=6000 at 20-page cap 2026-09-12; do not invent older",
}

# Team-fit honesty: cite #117/#118 terminals on SAME windows from committed reports.
# Do not re-invent. Known measured (do not alter):
#   #117 PUMP terminal +€6.47486669 vs BH +€19.29741176
#   #118 PUMP terminal +€13.44764028 vs BH +€19.29741176
PRIOR_TEAM_TERMINALS: dict[str, dict[str, dict[str, float]]] = {
    "117": {
        "PUMP-USDC": {
            "terminal_liquidation_net_eur": 6.47486669,
            "bh_net_return_eur": 19.29741176,
        },
        "TRUMP-USDC": {
            "terminal_liquidation_net_eur": -0.69853375,
            "bh_net_return_eur": -7.82566099,
        },
        "WIF-USDC": {
            "terminal_liquidation_net_eur": -8.45720868,
            "bh_net_return_eur": -10.32736265,
        },
    },
    "118": {
        "PUMP-USDC": {
            "terminal_liquidation_net_eur": 13.44764028,
            "bh_net_return_eur": 19.29741176,
        },
        "TRUMP-USDC": {
            "terminal_liquidation_net_eur": -5.86138627,
            "bh_net_return_eur": -7.82566099,
        },
        "WIF-USDC": {
            "terminal_liquidation_net_eur": -7.14213157,
            "bh_net_return_eur": -10.32736265,
        },
    },
}
...
def candidate_id_for(inst_id: str) -> str:
    """New per-inst candidate id. Never S1 / never #59 / never #117 / never #118 ids."""
...
def make_strategy() -> ScalpDogeRsiMr1hV1:
    """RSI14 MR confirm_closed_only long/flat. Never short. No EMA / DT / RVOL."""
    assert STRATEGY_BAR == BAR
...
def _assert_candidate_id_ok(cid: str) -> None:
    ...
    for bad in FORBIDDEN_ID_SUBSTRINGS:
        if bad in low:
            raise ReplayError(f"forbidden id substring {bad!r} in {cid}")
...
def score_inst(
    ...
    *,
    ...
    """Walk locked RSI14 MR long/flat; report completed expectancy + accounting_v2 terminal."""
    ...
    if len(trade_bars) < MIN_TRADE_BARS:
        raise ReplayError(
            f"{inst_id}: insufficient trade bars n={len(trade_bars)} "
            f"(need>={MIN_TRADE_BARS})"
        )
    walk = walk_long_flat(
        bars,
        strategy=strategy,
...
def run_rsi_mr_score(
    ...
    *,
    ...
    """Download three spot 1H series and score locked RSI14 MR. Fail closed on error."""
    ...
    errors: list[str] = []
    cache_dir = Path(data_dir) / "paper" / "candles" / "public_md_119"
    ...
    return {
        ...
        "rule": (
            "1H RSI(14) Wilder; entry = cross-up from ≤30 (prev ≤30 and curr >30); "
            "exit = closed RSI ≥70 → flat; no EMA filter; never short; "
            "confirm_closed_only"
        ),
        "bar": BAR,
        "rsi_period": RSI_PERIOD,
        ...
        "mechanism_reuse": "atlas.strategy.scalp_doge_rsi_mr_1h (#59 plumbing)",
        "team_fit": (
            "complement to Mid #71 4H breakout-trend + Scalp EMA #117 / "
            "Breakout #118 — mean-reversion payoff, not a trend clone"
        ),
        "prior_team_terminals_same_windows": PRIOR_TEAM_TERMINALS,
        "not_ema_family": True,
        "not_breakout_family": True,
        "not_s1": True,
        "not_59_panel_eur": True,
        "soft_pass_applied_as_arm": False,
        "soft_pass_status": "N/A_not_an_arm",
        ...
        out.append(
            {
                ...
                "end_equity_eur": r.get("end_equity_eur"),
                "bh_end_equity_eur": r.get("bh_end_equity_eur"),
                "win_rate": r.get("win_rate"),
                "time_in_market": r.get("time_in_market"),
                "error": r.get("error"),
            }
        )
    return out
