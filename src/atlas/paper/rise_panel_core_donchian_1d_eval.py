"""rise_panel_v1 Core #70 — DOGE 1D Donchian 20/10 €140 vs Core EMA12/30 baseline.
...
Canonical Donchian (DonchianLongFlatV1 / Mid #64 / Scalp #61 — entry > prior 20-high;
exit < prior 10-low). soft_promote_v1 gate unchanged. Compare Core vs Core-EMA-baseline.
Promote-as-better only if panel_net higher than Core EMA (often thin n / BH-like).
On PASS + better: Core(Donchian €140)+Mid(Breakout #65 €40) book €180 (no Scalp).
...
SOURCE = "rise_panel_v1_core_donchian_70"
WARMUP_DAILY = 40
...
CORE_MID_BOOK_ID = "rise_panel_v1_core_mid_book_180_core_donchian20_10_1d"
...
def run_core_donchian_1d(
    ...
    *,
    ...
    """Core Donchian 20/10 1D long/flat on SAME locked 7 — candidate #70."""

    def factory() -> CoreDogeDonchian1dV1:
        return CoreDogeDonchian1dV1()

    out = _run_panel(
        cfg,
        data_dir=data_dir,
        strategy_factory=factory,
        arm="core_donchian20_10_1d",
...
def deltas_vs_core_baseline(baseline: dict[str, Any], improve: dict[str, Any]) -> dict[str, Any]:
    """Panel metric deltas: Donchian Core − Core EMA baseline."""
...
def run_core_mid_book(
    ...
    *,
    ...
    """Core €140 1D Donchian 20/10 + Mid €40 4H BreakoutV1 — book €180, NO Scalp.
    ...
    return {
        ...
        "no_scalp": True,
        "note": (
            "Core(Donchian €140)+Mid(Breakout €40) only (bot cascade/arming path). "
...
def render_results_markdown(
    ...
    *,
    ...
    lines.append(
        ...
        "**Canonical Donchian 20/10** (Mid #64 / Scalp #61 / DonchianLongFlatV1) on **1D** Core — **no** EMA filter. Promote-as-better only if panel_net > Core EMA."
    )
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    lines.append(
        f"**Canonical Donchian:** repo `DonchianLongFlatV1` entry **{ENTRY_LOOKBACK}** / "
        f"exit **{EXIT_LOOKBACK}** (yaml untouched). Decision bar **1D**. Long/flat only. "
        "**No** EMA filter. Same family as Mid #64 / Scalp #61 paper rule."
    )
    lines.append("")
    ...
    lines.append(
        ...
        "Core #69 BreakoutV1 soft FAIL / worse panel_net. This trial ports **canonical Donchian "
        "20/10** (Mid #64 / Scalp #61 / DonchianLongFlatV1) onto Core €140 / locked rise panel / "
        "**1D** — without EMA so the family stays distinct from Core EMA and #69 Breakout."
    )
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    lines.append("")
    lines.append("- Script: `scripts/run_rise_panel_core_donchian_1d_eval.py`")
    lines.append("- Module: `atlas.paper.rise_panel_core_donchian_1d_eval`")
    lines.append(
        "- Strategy: `atlas.strategy.core_doge_donchian_1d` "
        "(thin Core wrapper around canonical `DonchianLongFlatV1` 20/10)"
    )
    lines.append(
        "- Reuse: `walk_long_flat`, `soft_promote_score`, locked `RISE_PANEL_V1` windows; "
        "Core(Donchian)+Mid(Breakout) book €180 on PASS+better only (no Scalp)"
    )
    lines.append("- Unit tests: `tests/unit/test_rise_panel_core_donchian_1d.py`")
    lines.append("")
    lines.append("---")
    lines.append("")
    lines.append("## D. Results — Core Donchian 20/10 1D €140 on same 7")
    lines.append("")
    lines.append(f"**core_improve_id:** `{CORE_IMPROVE_ID}`")
    lines.append("")
    ...
    lines.append("")
    lines.append("**Panel summary (Core Donchian 20/10 1D):**")
    lines.append(
    ...
    lines.append("")
    lines.append(
        f"### Soft promote (Core Donchian 20/10): **{soft.get('verdict', '—')}** "
        f"(`{SOFT_PROMOTE_GATE}`)"
    )
    lines.append("")
    lines.append(
    ...
    lines.append("")
    lines.append("### Honesty deltas vs Core 1D EMA baseline (Donchian − baseline)")
    lines.append("")
    if panel_worse:
        lines.append(
        ...
        lines.append("")
    lines.append("| Metric | Core EMA 1D €140 | Core Donchian 20/10 1D €140 | Δ |")
    lines.append("|--------|-----------------:|------------------------:|--:|")
    for key, label in (
    ...
    if promote_better and core_mid_book is not None:
        lines.append(
            "## E. Cascade / Core+Mid book (PASS + better → Core Donchian + Mid Breakout, no Scalp)"
        )
        lines.append("")
        lines.append(
        ...
        lines.append(f"- **core_id:** `{core_mid_book.get('core_id')}` (1D Donchian 20/10)")
        ...
        lines.append("| Sleeve | Core EMA + Mid BO ref | #70 Core Donchian + Mid BO | Δ |")
        lines.append("|--------|----------------------:|---------------------:|--:|")
        lines.append(
        ...
        lines.append("")
        lines.append(
            "Reports: `data/reports/rise_panel_v1_core_mid_book_core_donchian_70.json`"
        )
        lines.append("")
        lines.append("---")
        lines.append("")
    ...
    lines.append("- Not Core #69 BreakoutV1 (lookback 16 + ATR) — this is Donchian 20/10.")
