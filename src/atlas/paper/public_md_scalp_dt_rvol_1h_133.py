"""Public-MD Scalp #133 — #132 entry + 4H EMA21 regime-flip exits (single score).
...
Do NOT edit phase1/120–132 or config/default.yaml.
Do NOT import scalp_structure_bos_*. Pre-registered strengthen of **exits**, not N/k/RVOL.
...
PHASE1 = 133
SOURCE = "public_md_scalp_dt_rvol_1h_133"
ID_FAMILY = "public_md_v1_dt_n20_k0505_rvol20_gt10_ema21_4h_regime_exit_1h"
...
# Honesty baselines (not score transplants) — #132 FULL from results JSON
BASELINE_132_FULL: dict[str, dict[str, float | int]] = {
    "BTC-USDT": {
        "n": 22,
        "exp": -0.11740487,
        "terminal": -2.58290714,
...
BASELINE_132_BH_FULL: dict[str, float] = {
...
def _assert_candidate_id_ok(cid: str) -> None:
    ...
    if not cid.startswith(ID_FAMILY + "_"):
        raise ReplayError(f"candidate id must be dt+rvol 133 scoped: {cid}")
...
def walk_dt_rvol_1h_133(
    bars_1h: list[Bar],
    signals: DtRvol133Signals,
    *,
    ...
    """Long-only DT+RVOL walker: SL / 4H EMA regime-flip / time-stop 168.
    ...
    SL = SellLine at entry (fixed); ATR fallback; skip if SL not strictly below.
    NO 1.5R TP. NO current SellLine profit exit. n_tp and n_sellline stay 0.
    """
    bars = bars_1h
    if not bars:
    ...
    n_short_entries = 0
    n_skipped_sl = 0
    n_tp_exits = 0  # locked 0 — 1.5R off
    n_sl_exits = 0
    n_sellline_exits = 0  # locked 0 — sellline profit exit off
    ...
    for i, bar in enumerate(bars):
        ...
        if qty > 0.0 and entry_px > 0.0 and sl_px > 0.0:
            c = float(bar.close)
            # Priority: fixed SL (same close/next-open as #132), regime flip, time-stop.
            # NO 1.5R. NO current SellLine profit exit.
            if c <= sl_px:
...
def score_cell(
    ...
    *,
    ...
    return {
        ...
        "status": "MEASURED",
        "family_key": FAMILY,
        "family_label": "Dual Thrust N20 + RVOL>1.0 1H + 4H EMA21 regime-flip exits",
        "inst_id": inst_id,
        "window_key": window_key,
        ...
        rows.append(
            {
                "label": label,
                "inst_id": inst,
                "n_trades_133": int(c.get("n_trades") or 0),
                "n_trades_baseline": b.get("n"),
                "exp_133": exp_c,
                "exp_baseline": exp_b,
                "terminal_133": term_c,
                "terminal_baseline": term_b,
                "n_sl_133": c.get("n_sl_exits"),
                "n_regime_flip_133": c.get("n_regime_flip_exits"),
                "n_time_stop_133": c.get("n_time_stop_exits"),
                "n_tp_133": c.get("n_tp_exits"),
                "n_sellline_133": c.get("n_sellline_exits"),
                "n_tp_baseline": b.get("n_tp"),
                "n_sl_baseline": b.get("n_sl"),
                "n_sellline_baseline": b.get("n_sellline"),
                "n_time_stop_baseline": b.get("n_time_stop"),
...
def run_dt_rvol_1h_133_score(
    ...
    *,
    ...
    strat = ScalpDtRvol1h133V1(ScalpDtRvol1h133Params())
    ...
    vs_132 = _vs_baseline(cells, BASELINE_132_FULL, label="vs_132")
    ...
    return {
        ...
        "source": SOURCE,
        "stance": (
            "Research / public-MD single score: #132 entry + 4H EMA21 regime-flip "
            "exits (no 1.5R, no sell-line TP, time-stop 168). not_a_forecast. "
            "Soft PASS N/A ≠ arm. Pre-registered strengthen of exits — NOT N/k/RVOL grind. "
            "Leave #130/#131/#132 STOP. Do NOT take 134."
        ),
        "family": FAMILY,
        "id_family": ID_FAMILY,
        "mechanism": "atlas.strategy.scalp_dt_rvol_1h_133",
        ...
        "cells": cells,
        "vs_132_full": vs_132,
        "baseline_132_full": BASELINE_132_FULL,
        "baseline_132_bh_full": BASELINE_132_BH_FULL,
        "full_cells": [
            {
                ...
                "n_regime_flip_exits": c.get("n_regime_flip_exits"),
        ...
        rows.append(
            {
                ...
                "n_regime_flip": c.get("n_regime_flip_exits"),
