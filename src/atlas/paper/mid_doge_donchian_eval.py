"""Mid DOGE-USDT 1D Donchian 20/10 long/flat eval — NO EMA.

Reuses mid_btc_donchian_ema / Donchian long-flat plumbing without EMA regime.
Holdout-exp Mid gate like #36 mid_daily_pullback_eval.score_mid (NOT core_style_return).
Research only. not_a_forecast. Does NOT mutate config/default.yaml. Never places orders.
Scalp OUT. Time stop 15d. No ATR stop. Dual-window PRIMARY_SET_A then ALT_SET_B.
"""
...
from atlas.strategy.mid_doge_donchian import (
    ENTRY_LOOKBACK,
    EXIT_LOOKBACK,
    TIME_STOP_BARS,
    MidDogeDonchianParams,
    MidDogeDonchianV1,
    TradeWindowGate,
)

SOURCE = "mid_doge_donchian"
SPOT_MD = "DOGE-USDT"
WARMUP_DAILY = 40

MID_TIME_STOP_BARS = TIME_STOP_BARS  # 15 trading days (match mid_btc_donchian_ema)
...
def mid_strategy() -> MidDogeDonchianV1:
    return MidDogeDonchianV1(MidDogeDonchianParams(sleeve="mid"))
...
def run_mid_slice(
    *,
    ...
    """Causal Mid walk: pad history for Donchian; NEW entries only inside trade_bars span."""
    if not trade_bars:
        return {
            "ok": False,
            "fail_closed": True,
    ...
    eng = PaperEngine(
        ...
        gated,
        journal=NullJournal(),
        run_id=f"mid-doge-donchian-{label}",
        data_dir="data",
    )
    ...
    *,
    ...
    return {
        ...
        "mid": {
            ...
            "family": "donchian_breakout_no_ema",
        },
        "scalp": {
            "out_of_trial": True,
            "note": "Scalp halted for this trial — Mid DOGE Donchian only",
        },
...
def run_set(
    ...
    *,
    ...
    return {
        ...
        "bar": "1D",
        "family": "donchian_breakout_no_ema",
        "prior_family_note": "≠ Core EMA12/30; ≠ #39 BTC Donchian+EMA; ≠ #41 core_style_return",
        ...
        "rules": {
            ...
            "mid": (
                f"Donchian long-only 1D DOGE-USDT; entry close > prior {ENTRY_LOOKBACK}-day high; "
                f"exit close < prior {EXIT_LOOKBACK}-day low OR time {MID_TIME_STOP_BARS}d; "
...
def _render_window(wr: dict[str, Any]) -> list[str]:
    ...
    mid = wr.get("mid") or {}
    lines.append(
        f"**Mid (DOGE Donchian {ENTRY_LOOKBACK}/{EXIT_LOOKBACK}; NO EMA; NO ATR; "
...
def render_set_markdown(bundle: dict[str, Any]) -> str:
    """Per-set report for data/reports/mid_doge_donchian_{A,B}.md."""
    lines: list[str] = []
    sid = bundle.get("set_id")
    lines.append(f"# Mid #42 DOGE Donchian — set {sid}")
    lines.append("")
...
def render_markdown(bundle_a: dict[str, Any], bundle_b: dict[str, Any] | None) -> str:
    lines: list[str] = []
    lines.append("# 42 — Mid DOGE Donchian 20/10 long/flat (NO EMA; Scalp OUT)")
    lines.append("")
    ...
    lines.append(
        "**Family:** Mid **plain Donchian breakout** long/flat on **DOGE-USDT 1D** (20/10). "
        "**≠ Core EMA12/30**. **≠ phase1/39 BTC Donchian+EMA**. Reuses Donchian / mid_btc_donchian "
        "plumbing **without** EMA. Scalp halted. Mid €40 sleeve."
    )
    lines.append("")
    ...
    lines.append("")
    lines.append("### Mid — Donchian breakout long (spot DOGE-USDT 1D, €40)")
    lines.append("")
    lines.append(f"- Entry: close > prior {ENTRY_LOOKBACK}-day high (Donchian breakout up). Long only.")
    lines.append(
        f"- Exit (NO ATR; NO EMA): close < prior {EXIT_LOOKBACK}-day low OR time stop "
