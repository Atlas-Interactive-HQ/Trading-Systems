...
Walker = #127 long-only FT walker with indicator-exit (RSI/Stoch/CCI).
Paper only. place_orders false. not_a_forecast. Soft PASS N/A ≠ arm.
Do NOT edit phase1/120–127 or config/default.yaml.
...
PHASE1 = 128
SOURCE = "public_md_scalp_structure_bos_128"
ID_FAMILY_PREFIX = "public_md_v1_structure_bos_15m_ind_ladder_1m_long_only_ft"
PARENT_PHASE1 = 126
...
def _assert_candidate_id_ok(cid: str, rung_id: str) -> None:
    ...
    if not cid.startswith(ID_FAMILY_PREFIX + "_"):
        raise ReplayError(f"candidate id must be structure-bos 128 scoped: {cid}")
...
def walk_structure_bos_128(
    ...
    *,
    ...
    indicator_exit_min: float,
    indicator_kind: str,
) -> dict[str, Any]:
    """Long-only BOS walker: SL / 1.5R TP / opp-BOS / indicator-exit / time-stop.

    Indicator exit on first in-force value ≥ threshold (RSI / Stoch %K / CCI).
    ...
    if indicator_exit_min is None:
        raise ReplayError("indicator layer required: exit threshold missing")
    ...
    for i, bar in enumerate(bars):
        ...
        if qty != 0.0 and entry_px > 0.0 and sl_px > 0.0:
            ...
            if qty > 0.0:
                ...
                if not want_exit and indicator_exit_triggered(
                    signals.rsi[i], indicator_exit_min
                ):
                    want_exit, exit_reason = True, "indicator_exit"
    ...
    out = {
        ...
        "n_rsi_exits": n_indicator_exits if indicator_kind == "rsi" else 0,
        "n_time_stop_exits": n_time_exits,
        "r_multiple": float(r_multiple),
        "time_stop_bars": int(time_stop_bars),
        "time_stop_convention": TIME_STOP_CONVENTION,
        "indicator_exit_min": float(indicator_exit_min),
        "indicator_kind": indicator_kind,
        "leverage": settings.leverage,
        "walker": "walk_structure_bos_128",
        "not_a_forecast": True,
        "place_orders": False,
    }
    last_in = None
...
def score_cell(
    ...
    *,
    ...
    walk = walk_structure_bos_128(
        bars_1m,
        signals,
        settings=settings,
        trade_start_ms=window_start_ms,
        trade_end_ms=window_end_ms,
        indicator_exit_min=float(rung.exit_threshold),
        indicator_kind=rung.indicator_kind,
    )
    ...
    return {
        "ok": True,
        "status": "MEASURED",
        "family_key": f"structure_bos_128_{rung_id}",
        ...
        "indicator_kind": rung.indicator_kind,
        "indicator_tf": rung.indicator_tf,
        "rsi_period": rung.rsi_period,
        "exit_threshold": rung.exit_threshold,
        "late_filter_rsi_ge": rung.late_filter_rsi_ge,
        "pullback_dip_le": rung.pullback_dip_le,
        "entry_rsi_band": None,
        "window_start_iso": start_iso,
        "window_end_exclusive_iso": end_iso,
        "candidate_id": cid,
        "id_family": ID_FAMILY_PREFIX,
        "mechanism": "atlas.strategy.scalp_structure_bos_128",
        "reuses_strategy_helpers": "atlas.strategy.scalp_structure_bos_125",
        "reuses_cache": CACHE_DIR_NAME,
        "walker": "walk_structure_bos_128",
        "long_only": True,
        "bos_follow_through": True,
        "indicator_layer_present": True,
        "bar_entry": BAR_ENTRY,
...
def _vs_126_for_rung(
    ...
    for c in cells:
        ...
        exp_p = p.get("expectancy_completed_eur")
        term_c = c.get("terminal_liquidation_net_eur")
        term_p = p.get("terminal_liquidation_net_eur")
        beats_exp = (
            exp_c is not None
        ...
        rows.append(
            {
                ...
                "window_key": "FULL",
                "n_trades_128": int(c.get("n_trades") or 0),
                "n_trades_126": int(p.get("n_trades") or 0) if p else None,
                "exp_128": exp_c,
                "exp_126": exp_p,
                "terminal_128": term_c,
                "terminal_126": term_p,
                ...
                "exit_mix_128": {
                    ...
                    "indicator_exit": c.get("n_indicator_exits"),
                ...
                "pair_pass_full_128": c.get("pair_pass_full"),
...
def run_structure_bos_128_score(
    cfg: Any,
    *,
    data_dir: Path,
    ...
    try:
        ...
        for rid in selected:
            _strat = StructureBos128V1(StructureBos128Params(rung_id=rid))  # type: ignore[arg-type]
            for inst in USDT_INSTS:
    ...
    return {
        ...
        "family": FAMILY,
        "strategy": "atlas.strategy.scalp_structure_bos_128",
        "reuses_strategy_helpers": "atlas.strategy.scalp_structure_bos_125",
        "reuses_cache": CACHE_DIR_NAME,
        "walker": "walk_structure_bos_128",
        "long_only": True,
        ...
        "ladder_stop": True,
        "ladder_stop_note": "STOP after R1–R8; do not take 129; no off-ladder grind",
