"""Daily EMA + Donchian confirm paper eval. Parallel research — not Phase A.
...
CONFIRM_SOURCE = "ema-donchian-confirm"
CONFIRM_ASSET = "BTC-USDT"


def evaluate_window_ema_donchian(
    *,
    window: Any,
    bars: list[Bar],
    strategy: EmaDonchianConfirmV1,
    ...
    split: dict[str, Any] = {
        ...
        "rule": (
            ...
            "EMA + Donchian use pad+prior bars (causal, exclusive Donchian lookback)."
        ),
    }
    if not holdout_ok:
    ...
    return {
        ...
        "sample_id": window.id,
        "symbol": symbol,
        "md_label": f"research MD {symbol} 1D EMA+Donchian confirm; window {window.label}",
        ...
        "holdout": ho,
        "disclaimer": (
            "research only. not_a_forecast. EMA 12/30 AND Donchian 20/10 confirm. "
...
def run_ema_donchian_eval(
    cfg: Any,
    *,
    asset: str = CONFIRM_ASSET,
    windows: str,
    ...
    for win in specs:
        try:
            ...
            row = evaluate_window_ema_donchian(
        ...
        results.append(row)
        out = reports / f"ema_donchian_{asset}_{win.id}.json"
        out.write_text(
    ...
    bundle = {
        ...
        "errors": errors,
        "disclaimer": (
            "research only. not_a_forecast. EMA 12/30 AND Donchian 20/10 confirm on BTC-USDT 1D. "
            "dual-window interesting must both clear or FAIL. CLEAR/FAIL are documentation only — "
            "do not promote. not a Phase C or live gate. does not replace Phase A, live20, "
            "or the EMA 12/30 observer."
        ),
    }
    (reports / f"ema_donchian_bundle_{asset}.json").write_text(
...
def render_ema_donchian_markdown(bundle: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "Do not promote to Phase C or live. Does **not** replace Phase A BreakoutV1, live20, or the "
        ...
        f"**closed close > prior {bundle.get('entry_lookback')}-day high** (exclusive lookback). "
        f"Flat iff **EMA({bundle.get('fast')}) ≤ EMA({bundle.get('slow')})** OR "
        f"**closed close < prior {bundle.get('exit_lookback')}-day low**. Never short. "
        ...
        "",
        "Locked weekday observer is still **EMA 12/30 long/flat** under `data/ema/` (PR #13). "
        "PR #18 is **Donchian 20/10** long/flat without the EMA filter. This trial is the AND of both "
        "entry conditions and the OR of both exit conditions. It does not change either default.",
        "",
        "**Bull-window selection bias:** 2020-09 and 2023-09 include historically strong crypto bull legs. "
        "A long-only confirm rule is advantaged here. That is not a forecast.",
        "",
    ...
    lines.extend(
        [
            ...
            "```",
            "",
            "Writes `ema_donchian_{asset}_{win}.json` under `data/reports/` — does **not** overwrite "
            "EMA `ema_*`, Donchian `donchian_*`, or 12/21 compare reports.",
            "",
            "## What this is not",
            "",
