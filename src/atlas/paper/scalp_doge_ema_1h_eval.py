"""Scalp DOGE-USDT 1H EMA12/30 long/flat + daily bull — Core-style RETURN (#48).

LOCKED #48. Scalp €20 sleeve. Gate: core_style_return (intentional; differs from
...
DIFFERS_REASON = (
    ...
    "(Mid #41 / #45 / Scalp #46 pattern) scaled to Scalp €20 instead of "
...
SAME_RULE_AS = (
    "Mid #45 / mid_doge_ema_coregate (#41) / Core ema12_30 — Scalp €20 on 1H "
    "+ daily EMA bull entry gate (#46 spirit)"
)
...
def _holdout_window(window: CascadeWindow, hold_bars: list[Bar]) -> CascadeWindow:
    if not hold_bars:
        raise ReplayError("empty holdout for scalp doge ema 1h")
...
def run_scalp_ema_slice(
    *,
    ...
    """Full-sleeve EMA12/30 long/flat on Scalp equity (DOGE-USDT 1H + daily bull)."""
    settings = EmaBookSettings(
    ...
    strat = ScalpDogeEma1hV1(
        ScalpDogeEma1hParams(fast=FAST, slow=SLOW, daily_bull_filter=True),
...
def evaluate_window(
    ...
    *,
    ...
    return {
        ...
        "mid": {
            "out_of_trial": True,
            "note": "Mid halted for this trial — Scalp DOGE EMA 1H Core-style return gate only",
        },
        "core": {
            "out_of_trial": True,
    ...
    return {
        ...
        "rules": {
            "scalp": (
                ...
                f"full-sleeve long/flat on Scalp €{SCALP_START_EUR:.0f} DOGE-USDT {BAR}; "
...
def _render_window(wr: dict[str, Any]) -> list[str]:
    ...
    lines.append(
        f"**Scalp (DOGE EMA{FAST}/{SLOW} long/flat + daily EMA bull entry gate; "
...
def render_set_markdown(bundle: dict[str, Any]) -> str:
    ...
    lines.append(f"# Scalp #48 DOGE EMA 1H + daily bull — set {sid}")
    lines.append("")
...
def render_markdown(bundle_a: dict[str, Any], bundle_b: dict[str, Any] | None) -> str:
    ...
    lines.append(
        "**Family:** Scalp **EMA12/30 long/flat** on **DOGE-USDT 1H** + **daily EMA12>EMA30** "
        "filter ON for **new longs** (bull focus — entry gate); sleeve €20. SAME Core EMA rule "
        "spirit (`EmaTrendV1` / Mid #41 / Mid #45) scaled to Scalp €20 on 1H. **Gate:** Core-style "
    ...
    lines.append("")
    lines.append("### Scalp — EMA12/30 long/flat + daily bull (spot DOGE-USDT **1H**, €20)")
    lines.append("")
    ...
    lines.append(
        ...
        "(daily flip alone does not force exit — entry gate, mirrors #46 / PullbackLongV1)."
    )
    ...
    lines.append("")
    lines.append("- Do **not** change EMA periods, sleeve size, bar size, daily filter, or costs to chase PASS.")
