"""Public-MD Scalp #131 — Dual Thrust N4 + RVOL 1H + 4H EMA21 (single score).
...
Do NOT edit phase1/120–130 or config/default.yaml.
Do NOT import scalp_structure_bos_*. Leave #130 15m STOP. Do NOT take 132.
...
PHASE1 = 131
SOURCE = "public_md_scalp_dt_rvol_1h_131"
ID_FAMILY = "public_md_v1_dt_n4_k0505_rvol20_gt12_ema21_4h_regime_1h"
...
BASELINE_130_FULL: dict[str, dict[str, float | int]] = {
    "BTC-USDT": {"n": 296, "exp": -0.0339961, "terminal": -10.06284485},
    "ETH-USDT": {"n": 321, "exp": -0.02545166, "terminal": -8.16998247},
    "DOGE-USDT": {"n": 275, "exp": -0.03558125, "terminal": -9.78484417},
}
...
def _assert_candidate_id_ok(cid: str) -> None:
    ...
    if not cid.startswith(ID_FAMILY + "_"):
        raise ReplayError(f"candidate id must be dt+rvol 131 scoped: {cid}")
...
def walk_dt_rvol_1h_131(
    bars_1h: list[Bar],
    signals: DtRvol131Signals,
    *,
...
def load_or_fetch_1h_4h(
    ...
    *,
    ...
    """Return (bars_1h, bars_4h, meta, client). Reuse 121 1H; 131 4H or resample."""
...
def score_cell(
    ...
    *,
    ...
    return {
        ...
        "status": "MEASURED",
        "family_key": FAMILY,
        "family_label": "Dual Thrust N4 + RVOL>1.2 1H + 4H EMA21 regime",
        "inst_id": inst_id,
        "window_key": window_key,
        ...
        "candidate_id": cid,
        "id_family": ID_FAMILY,
        "mechanism": "atlas.strategy.scalp_dt_rvol_1h_131",
        "bar": BAR,
        ...
        rows.append(
            {
                "label": label,
                "inst_id": inst,
                "n_trades_131": int(c.get("n_trades") or 0),
...
def run_dt_rvol_1h_131_score(
    ...
    *,
    ...
    strat = ScalpDtRvol1h131V1(ScalpDtRvol1h131Params())
    ...
    vs_130 = _vs_baseline(cells, BASELINE_130_FULL, label="vs_130")
    ...
    return {
        ...
        "source": SOURCE,
        "stance": (
            "Research / public-MD single score: 1H Dual Thrust N4 + RVOL>1.2 "
            "+ 4H EMA21 regime. not_a_forecast. Soft PASS N/A ≠ arm. "
            "NEW card — leave #130 15m STOP; not a #121 transplant; not a TF grind."
        ),
        "family": FAMILY,
        "id_family": ID_FAMILY,
        "mechanism": "atlas.strategy.scalp_dt_rvol_1h_131",
        ...
        "baseline_130_full": BASELINE_130_FULL,
