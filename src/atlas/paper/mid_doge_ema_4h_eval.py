"""Mid DOGE-USDT 4H EMA12/30 long-only — Core-style RETURN gate at Mid €40.

LOCKED #45. SAME EMA12/30 long/flat rule as mid_doge_ema_coregate (#41) /
EmaTrendV1 / walk_long_flat, but on **4H** bars (TF Mid ≠ 1D Core twin #41).
...
from atlas.strategy.mid_doge_ema_4h import BAR, FAMILY, FAST, SLOW

SOURCE = "mid_doge_ema_4h"
SPOT_MD = "DOGE-USDT"
...
SAME_RULE_AS = "mid_doge_ema_coregate (#41) / Core ema12_30_long_only — TF Mid 4H ≠ 1D twin"
...
def _holdout_window(window: CascadeWindow, hold_bars: list[Bar]) -> CascadeWindow:
    if not hold_bars:
        raise ReplayError("empty holdout for mid doge ema 4h")
...
def run_mid_ema_slice(
    *,
    ...
    """Full-sleeve EMA12/30 long/flat on Mid equity (DOGE-USDT 4H)."""
    ...
    *,
    ...
    return {
        ...
        "mid": {
            ...
            "differs_from_holdout_exp_gate": True,
            "differs_reason": DIFFERS_REASON,
            "tf_note": "TF Mid 4H ≠ 1D Core twin #41",
        },
        "scalp": {
            "out_of_trial": True,
            "note": "Scalp halted for this trial — Mid DOGE EMA 4H Core-style return gate only",
        },
    ...
    return {
        ...
        "rules": {
            "mid": (
                ...
                f"full-sleeve long/flat on Mid €{MID_START_EUR:.0f} DOGE-USDT {BAR}; "
...
def _render_window(wr: dict[str, Any]) -> list[str]:
    ...
    mid = wr.get("mid") or {}
    lines.append(
        f"**Mid (DOGE EMA{FAST}/{SLOW} long/flat full-sleeve €{MID_START_EUR:.0f} on **{BAR}**; "
...
def render_set_markdown(bundle: dict[str, Any]) -> str:
    ...
    lines.append(f"# Mid #45 DOGE EMA 4H — set {sid}")
    lines.append("")
...
def render_markdown(bundle_a: dict[str, Any], bundle_b: dict[str, Any] | None) -> str:
    lines: list[str] = []
    lines.append("# 45 — Mid DOGE EMA12/30 on **4H** Core-style RETURN gate (Mid sleeve; Scalp OUT)")
    lines.append("")
    ...
    lines.append(
        "**Family:** Mid **EMA12/30 long/flat** on **DOGE-USDT 4H** — SAME Core rule "
        f"(`EmaTrendV1` / `walk_long_flat` / #41 spirit) at Mid €40. **TF Mid ≠ 1D Core twin #41**. "
    ...
    lines.append("")
    lines.append("### Mid — EMA12/30 long/flat (spot DOGE-USDT **4H**, €40)")
    lines.append("")
    ...
    lines.append(f"- Bar: **{BAR}** (TF Mid ≠ 1D Core twin #41).")
    lines.append("- Low n_trades: **OK** (document n_trades / TIM / expectancy; not a FAIL gate).")
    lines.append("- Windows: same A/B calendars as recent Mid trials mapped to 4H bars; MD fallbacks labeled.")
    lines.append("")
    lines.append("### Core (informational only)")
    lines.append("")
    lines.append("- Same rule on DOGE-USDT €140 **1D** reported as context only.")
    ...
    lines.append(
        ...
        "Mirrors scoring spirit of mid_doge_ema_coregate (#41) adapted to this 4H trial. "
    ...
    lines.append("- On FAIL: archive; report only — **no** EMA period / TF / costs grind.")
