"""Public-MD Scalp #130 — Dual Thrust N4 + RVOL 15m + 1H EMA21 (single score).

Reuse 15m cache public_md_125_cache + 1H cache public_md_121.
Paper only. place_orders false. not_a_forecast. Soft PASS N/A ≠ arm.
Do NOT edit phase1/120–129 or config/default.yaml.
Do NOT import scalp_structure_bos_*.
...
PHASE1 = 130
SOURCE = "public_md_scalp_dt_rvol_15m_130"
ID_FAMILY = "public_md_v1_dt_n4_k0505_rvol20_gt12_ema21_1h_regime_15m"
...
MIN_TRADE_BARS_FULL = 15_000
MIN_TRADE_BARS_SUB = 7_000
PASS_PAIRS_NEEDED = 2
...
def _assert_candidate_id_ok(cid: str) -> None:
    ...
    if not cid.startswith(ID_FAMILY + "_"):
        raise ReplayError(f"candidate id must be dt+rvol 130 scoped: {cid}")
...
def walk_dt_rvol_15m_130(
    bars_15m: list[Bar],
    signals: DtRvol130Signals,
    *,
    ...
    if any(not b.closed for b in bars):
        raise ReplayError("open/partial 15m bar (fail closed)")
    n = len(bars)
    if (
...
def _validate_trade_coverage(bars: list[Bar], inst_id: str, bar: str) -> None:
    ...
    min_need = MIN_TRADE_BARS_FULL if bar == "15m" else 4000
...
def fetch_series(
    ...
    *,
    ...
    max_pages = MAX_HISTORY_PAGES_15M if bar == "15m" else MAX_HISTORY_PAGES_1H
    try:
...
def load_or_fetch_15m_1h(
    ...
    *,
    ...
    """Return (bars_15m, bars_1h, meta, client). Reuse 125/121 caches."""
    meta: dict[str, Any] = {"15m_source": None, "1h_source": None}
    start_ms = iso_to_ms(WARMUP_START_ISO)
    end_ms = iso_to_ms(FETCH_END_EXCLUSIVE_ISO)
    own = False
    http = client

    path_15 = cache_path_15m(results_dir, inst_id)
    bars_15: list[Bar] = []
    if use_cache and path_15.is_file() and path_15.stat().st_size > 100_000:
        bars_15 = load_jsonl_candles(path_15, symbol=inst_id, bar=BAR)
        bars_15 = _filter_window(bars_15, start_ms, end_ms)
        try:
            _validate_trade_coverage(bars_15, inst_id, "15m")
    ...
    if use_cache and path_1h.is_file() and path_1h.stat().st_size > 50_000:
        bars_1h = load_jsonl_candles(path_1h, symbol=inst_id, bar=REGIME_BAR)
    ...
    if not bars_15 or not bars_1h:
        if http is None:
            http = httpx.Client(headers={"User-Agent": USER_AGENT}, timeout=60.0)
            own = True
        if not bars_15:
            bars_15 = fetch_series(
                http, inst_id, BAR, rest_base=rest_base, pause_s=pause_s
            )
            path_15.parent.mkdir(parents=True, exist_ok=True)
            persist_candles(path_15, bars_15)
    ...
    meta["n_15m"] = len(bars_15)
    meta["n_1h"] = len(bars_1h)
    meta["own_client"] = own
    return bars_15, bars_1h, meta, http
...
def score_cell(
    ...
    *,
    ...
    trade_bars = [
        b for b in bars_15m if window_start_ms <= b.ts_open_ms < window_end_ms
    ]
    ...
    return {
        ...
        "status": "MEASURED",
        "family_key": FAMILY,
        "family_label": "Dual Thrust N4 + RVOL>1.2 15m + 1H EMA21 regime",
        "inst_id": inst_id,
        "window_key": window_key,
        ...
        "candidate_id": cid,
        "id_family": ID_FAMILY,
        "mechanism": "atlas.strategy.scalp_dt_rvol_15m_130",
        "bar": BAR,
        ...
        "no_martingale": True,
        "no_leverage": True,
        "n_bars_fetched_incl_warmup": len(bars_15m),
        ...
        "reuses_15m_cache": CACHE_15M_DIR_NAME,
        ...
        rows.append(
            {
                "label": label,
                "inst_id": inst,
                "n_trades_130": int(c.get("n_trades") or 0),
...
def run_dt_rvol_15m_130_score(
    ...
    *,
    ...
    (results_dir / CACHE_15M_DIR_NAME).mkdir(parents=True, exist_ok=True)

    window_keys = list(WINDOWS.keys()) if include_subs else ["FULL"]
    http = client
    fetch_errors: list[str] = []
    probe_meta: dict[str, Any] = {}
    series_15: dict[str, list[Bar]] = {}
    series_1h: dict[str, list[Bar]] = {}
    cells: list[dict[str, Any]] = []
    owned_clients: list[httpx.Client] = []

    strat = ScalpDtRvol15m130V1(ScalpDtRvol15m130Params())

    try:
        for inst in USDT_INSTS:
            try:
                b15, b1h, meta, http = load_or_fetch_15m_1h(
                    http,
                    inst,
                ...
                if meta.get("own_client") and http is not None and http not in owned_clients:
                    # client may be shared; close only if we created and caller didn't pass
                    pass
        ...
        for inst in USDT_INSTS:
            bars = series_15.get(inst) or []
            bars_1h = series_1h.get(inst) or []
            if not bars or not bars_1h:
                err = next(
                ...
                continue
            signals = strat.precompute_signals(bars, bars_1h)
            for window_key in window_keys:
                cells.append(
                    score_cell(
                        bars,
    ...
    return {
        ...
        "source": SOURCE,
        "stance": (
            "Research / public-MD single score: 15m Dual Thrust N4 + RVOL>1.2 "
            "+ 1H EMA21 regime. not_a_forecast. Soft PASS N/A ≠ arm. "
            "NEW family — not a BOS extension, not a #121 transplant."
        ),
        "family": FAMILY,
        "id_family": ID_FAMILY,
        "mechanism": "atlas.strategy.scalp_dt_rvol_15m_130",
        ...
        "no_bos_import": True,
        "no_param_grind": True,
        "series_n_bars_15m": series_n_15,
        "series_n_bars_1h": series_n_1h,
        "series_n_bars_full_trade_15m": trade_n,
        ...
        "baseline_126_full": BASELINE_126_FULL,
...
__all__ = [
    ...
    "WINDOWS",
    "candidate_id_for",
    "load_or_fetch_15m_1h",
    "measured_table_rows",
    "run_dt_rvol_15m_130_score",
    "score_cell",
