"""Scalp DOGE-USDT 1H EMA12/30 long/flat + daily EMA12/30 bull entry gate (#48).

LOCKED Scalp #48. Same EMA12/30 long/flat spirit as Core / Mid #41 / Mid #45
but on **1H** bars at Scalp €20, with **daily EMA12>EMA30 filter ON for new
longs** (bull focus — entry gate; mirrors #46 / PullbackLongV1).
...
class ScalpDogeEma1hParams:
    fast: int = FAST
    slow: int = SLOW
    ema_fast_daily: int = FAST
    ema_slow_daily: int = SLOW
    confirm_closed_only: bool = True
    daily_bull_filter: bool = True  # ON for new longs (locked #48)
    sleeve: str = "scalp"
    bar: str = BAR


class ScalpDogeEma1hV1:
    ...
    def __init__(
        self,
        params: ScalpDogeEma1hParams | None = None,
        *,
        daily_bars: Sequence[Bar] | None = None,
    ) -> None:
        self.params = params or ScalpDogeEma1hParams()
        p = self.params
    ...
    def label(self) -> str:
        ...
        return f"scalp_doge_ema_1h_{p.sleeve}_{p.fast}_{p.slow}_{bull}"
