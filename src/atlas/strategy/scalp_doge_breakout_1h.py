"""Scalp DOGE-USDT 1H BreakoutV1 long/flat — rise_panel Scalp #62.

LOCKED Scalp #62. Canonical BreakoutV1 paper plumbing on 1H:
  ...
  Lookback = 16 (BreakoutParams.lookback_15m default, applied on 1H decision bar).
  oneh_filter: off (decision TF is already 1H).
  NO daily-bull EMA filter — keep family distinct from EMA twins / #46 bull gate.
  Never short. Never places orders. not_a_forecast.
Sleeve Scalp €20. Do not grind lookback / ATR / TF / costs on FAIL.
ATR stop from Signal-path BreakoutV1 is NOT applied in walk_long_flat harness
(channel exit only — same rise_panel long/flat pattern as #61).
"""
...
class ScalpDogeBreakout1hParams:
...
class ScalpDogeBreakout1hV1:
    """BreakoutV1 long/flat on DOGE-USDT 1H for Scalp sleeve (no daily bull).

    Thin Scalp wrapper around canonical BreakoutV1 channel + ATR quiet filter.
    Used with walk_long_flat (signal close → next open). Full-sleeve sizing at
    Scalp €20 is applied by the eval harness. Rejects lookback / TF / sleeve /
    oneh / bull-filter sweeps (no grind on FAIL).
    """

    def __init__(self, params: ScalpDogeBreakout1hParams | None = None) -> None:
        self.params = params or ScalpDogeBreakout1hParams()
        p = self.params
        ...
        if p.oneh_filter != "off":
            raise ValueError("oneh_filter must be 'off' for 1H BreakoutV1 (#62)")
        if p.bar != BAR:
    ...
    def label(self) -> str:
        p = self.params
        return (
            f"scalp_doge_breakout_1h_{p.sleeve}"
    ...
    def desired_state(self, bars: Sequence[Bar]) -> str:
        ...
        No daily-bull EMA. Never short. Quiet/ranging bars refuse NEW longs.
        """
        p = self.params
        state = FLAT
        if not bars:
