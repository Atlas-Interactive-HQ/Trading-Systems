"""Mid DOGE-USDT 4H RSI(14) mean-reversion long/flat — rise_panel Mid #66.

LOCKED Mid #66. Same entry/exit style as Scalp #59 (not Mid #43 1D level-entry):
...
Reuses Wilder RSI helper from mid_doge_rsi_mr.
Sleeve Mid €40. Do not grind RSI period / thresholds / TF / costs on FAIL.
"""
...
class MidDogeRsiMr4hV1:
    """RSI(14) mean-reversion long/flat on DOGE-USDT 4H for Mid sleeve.
    ...
    No EMA regime filter. Distinct from Mid EMA archive, Breakout #65, Donchian #64.
    """

    def __init__(self, params: MidDogeRsiMr4hParams | None = None) -> None:
        self.params = params or MidDogeRsiMr4hParams()
        p = self.params
    ...
    def label(self) -> str:
        p = self.params
        return (
            f"mid_doge_rsi_mr_4h_{p.sleeve}_rsi{p.rsi_period}"
