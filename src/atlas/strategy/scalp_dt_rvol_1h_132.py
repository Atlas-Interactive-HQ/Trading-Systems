"""Public-MD Scalp #132 — Dual Thrust N=20 + RVOL>1.0 1H + 4H EMA21 regime.
...
BAR = "1H"
REGIME_BAR = "4H"
FAMILY = "dt_n20_k0505_rvol20_gt10_ema21_4h_regime_1h"
SLEEVE = "scalp"
PHASE1 = 132


@dataclass(frozen=True)
class ScalpDtRvol1h132Params:
    lookback: int = N
    k1: float = K1
    k2: float = K2
...
@dataclass(frozen=True)
class DtRvol132Signals:
    """Per-1H-bar causal series for the #132 walker."""
...
class ScalpDtRvol1h132V1:
    """1H Dual Thrust N20 + RVOL>1.0 gated by 4H EMA21 (long-only)."""

    def __init__(self, params: ScalpDtRvol1h132Params | None = None) -> None:
        self.params = params or ScalpDtRvol1h132Params()
        p = self.params
        if (
            p.lookback != N
    ...
    def label(self) -> str:
        p = self.params
        return (
            f"scalp_dt_rvol_1h_132_n{p.lookback}_k{p.k1}_{p.k2}"
