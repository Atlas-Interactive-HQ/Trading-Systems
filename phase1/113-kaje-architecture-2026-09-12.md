...
| [`114`](./114-scalp-multi-coin-watch-2026-09-12.md) | Overlay (same date): Scalp may **watch multiple coins** under the **one** 10% bucket. PEPE remains preferred if **DEMO_CLEAR**. Catalogue CLEAR ≠ DEMO_CLEAR ≠ Soft PASS ≠ arm. Does **not** re-lock this architecture. |
...
Kaje overlay **2026-09-12**: Scalp may **watch multiple coins** under the **one** Scalp **10%** bucket (the **1** in **6 : 3 : 1**). PEPE remains the preferred target **if DEMO_CLEAR**. PEPE is **not** DEMO_CLEAR today. Alternatives are an **unranked** watch pool — Research ranks later. **No invented expectancy / order `instId`.** See [`114`](./114-scalp-multi-coin-watch-2026-09-12.md). This architecture lock is **unchanged**.
