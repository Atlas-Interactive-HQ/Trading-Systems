...
**Mechanical consequences, not edge.** (1) With a 2× wider stop the sizing rule (risk budget 1.5% of equity ÷ stop fraction) halves the notional per trade, so turnover and fee drag fall (holdout fee drag 39.23→23.71 € and 41.92→32.06 €). (2) Fewer stop-outs per UTC day means the 5% daily kill trips far less often (full-sample kill-days 52→7 and 38→9); the lower max DD partly reflects fewer forced flattenings. (3) Under 2× fees the candidate's less-negative rows are kill truncation again (kill-days 7→17 and 9→14) — flagged below, not a win.

**Decision boundary.** PASS is a research result on paper holdout. `config/default.yaml` is unchanged in this PR; whether `atr_stop_mult: 3.0` becomes the new frozen baseline is Atlas/Kaje's call, and Phase C / live are separate gates untouched by this trial.
<!-- run-note:end -->
