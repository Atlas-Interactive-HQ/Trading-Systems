#!/usr/bin/env python3
"""EMA 12/30 + SMA(200) regime (BTC-USDT 1D). Research only. No orders."""
...
from atlas.paper.ema_sma200_eval import (  # noqa: E402
    SMA200_ASSET,
    render_ema_sma200_markdown,
    run_ema_sma200_eval,
)
from atlas.paper.replay import ReplayError  # noqa: E402
from atlas.strategy.ema_sma200 import SMA_PERIOD, WARMUP_DAYS  # noqa: E402


def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description=(
            "EMA 12/30 + SMA(200) regime (BTC-USDT 1D). Research only. "
            "Never places orders. Does not replace Phase A or the EMA observer."
        )
    )
    p.add_argument("--config", default=None)
    p.add_argument("--data-dir", default=None)
    p.add_argument("--asset", default=SMA200_ASSET)
    p.add_argument(
        "--windows",
    ...
    p.add_argument("--sma-period", type=int, default=SMA_PERIOD)
    p.add_argument("--warmup-days", type=int, default=WARMUP_DAYS)
    p.add_argument("--pause-s", type=float, default=0.12)
    p.add_argument(
        "--write-md",
        default=None,
        help="Markdown path. Default phase1/30-ema-sma200-regime.md. Empty to skip.",
    )
    ...
    if write_md is None:
        write_md = str(_ROOT / "phase1" / "30-ema-sma200-regime.md")
    if write_md:
        md_path = Path(write_md)
        md_path.parent.mkdir(parents=True, exist_ok=True)
        md_path.write_text(render_ema_sma200_markdown(bundle), encoding="utf-8")
