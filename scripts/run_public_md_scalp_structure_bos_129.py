#!/usr/bin/env python3
"""Public-MD Scalp #129 — time-stop/R/ATR-SL/retest ladder on #126+FT Jul2020–Jan2021 €20.

Never places orders. Does NOT change config/default.yaml.
not_a_forecast. Soft PASS N/A ≠ arm. Do not edit phase1/120–128.
...
from atlas.paper.public_md_scalp_structure_bos_129 import (  # noqa: E402
    measured_table_rows,
    run_structure_bos_129_score,
    write_report_json,
)
from atlas.paper.replay import ReplayError  # noqa: E402
from atlas.strategy.scalp_structure_bos_129 import RUNG_IDS  # noqa: E402


def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description=(
            "Public-MD Scalp #129: time-stop/R/ATR-SL/retest ladder E1–E6 on #126+FT "
    ...
    p.add_argument(
        "--rungs",
        default=None,
        help="Comma-separated rung ids (default all E1–E6). On-ladder only.",
    )
    ...
    out_path = (
        Path(args.out)
        if args.out
        else reports / "public_md_scalp_structure_bos_129.json"
    )
    ...
    try:
        bundle = run_structure_bos_129_score(
            cfg,
            data_dir=data_dir,
    ...
    results_path = results_dir / "public_md_scalp_structure_bos_129.json"
