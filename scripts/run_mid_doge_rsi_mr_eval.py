#!/usr/bin/env python3
"""Mid DOGE RSI(14) mean-reversion long/flat eval. Research only. Never places orders.
...
from atlas.paper.mid_doge_rsi_mr_eval import (  # noqa: E402
    render_markdown,
    render_set_markdown,
    run_set,
    write_report_json,
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Mid DOGE RSI(14) MR eval (research only)")
    p.add_argument("--config", default=None)
    p.add_argument("--data-dir", default=None)
    p.add_argument("--pause-s", type=float, default=0.12)
    p.add_argument("--set", choices=("A", "B", "both"), default="both")
    p.add_argument("--write-md", default="phase1/43-mid-doge-rsi-mr.md")
    ...
    if args.set in ("A", "both"):
        ...
        path_a = write_report_json(bundle_a, reports / "mid_doge_rsi_mr_A.json")
        md_a = reports / "mid_doge_rsi_mr_A.md"
    ...
    if args.set in ("B", "both"):
        ...
        path_b = write_report_json(bundle_b, reports / "mid_doge_rsi_mr_B.json")
        md_b = reports / "mid_doge_rsi_mr_B.md"
    ...
    if bundle_a is None:
        existing_a = reports / "mid_doge_rsi_mr_A.json"
        if existing_a.is_file():
            bundle_a = json.loads(existing_a.read_text(encoding="utf-8"))
    if bundle_b is None:
        existing_b = reports / "mid_doge_rsi_mr_B.json"
