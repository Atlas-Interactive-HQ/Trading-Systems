...
from atlas.paper.rise_panel_scalp_rsi_ema1221_1h_eval import (  # noqa: E402
    SCALP_4H_ID,
    SCALP_15M_ID,
    SCALP_IMPROVE_ID,
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        ...
        default="both",
        help="provisional=#55 1H; improve=RSI∩EMA 1H; both=run+deltas; refs=also re-score #57 4H",
    )
    p.add_argument("--write-md", default="phase1/60-rise-panel-scalp-rsi14-mr-ema1221-1h.md")
    p.add_argument(
        "--skip-cascade",
        action="store_true",
    ...
    if run_4h:
        ...
        path_4 = write_report_json(
            scalp_4h, reports / "rise_panel_v1_scalp_4h_recheck_for_60.json"
        )
        print(
            json.dumps(
                {
    ...
    if run_imp:
        improve = run_scalp_rsi_ema1221_1h(cfg, data_dir=data_dir, pause_s=args.pause_s)
        path_i = write_report_json(
            improve, reports / "rise_panel_v1_scalp_rsi14_mr_ema1221_1h_60.json"
        )
        print(
            json.dumps(
                {
    ...
    if args.mode == "improve" and provisional is None:
        ...
        if existing.is_file():
            ...
            alt = Path(
                "/workspace/trading-system-scalp-59/data/reports/rise_panel_v1_scalp_provisional_recheck.json"
            )
            if alt.is_file():
                provisional = json.loads(alt.read_text(encoding="utf-8"))
    if scalp_4h is None:
        for cand in (
            reports / "rise_panel_v1_scalp_4h_improve.json",
            reports / "rise_panel_v1_scalp_4h_recheck_for_60.json",
            Path("/workspace/trading-system-scalp-57/data/reports/rise_panel_v1_scalp_4h_improve.json"),
            Path("/workspace/trading-system-scalp-59/data/reports/rise_panel_v1_scalp_4h_recheck_for_59.json"),
        ):
            if cand.is_file():
                scalp_4h = json.loads(cand.read_text(encoding="utf-8"))
                break

    # Load #59 RSI MR report for honesty deltas
    for cand in (
        reports / "rise_panel_v1_scalp_rsi14_mr_1h_59.json",
        Path("/workspace/trading-system-scalp-59/data/reports/rise_panel_v1_scalp_rsi14_mr_1h_59.json"),
    ):
        if cand.is_file():
            scalp_rsi_mr = json.loads(cand.read_text(encoding="utf-8"))
            break
    ...
    if provisional is not None and improve is not None:
        ...
        path_d = write_report_json(
            ...
            reports / "rise_panel_v1_scalp_rsi_ema1221_60_deltas_vs55.json",
        )
    ...
    if scalp_4h is not None and improve is not None:
        ...
        path_d57 = write_report_json(
            ...
            reports / "rise_panel_v1_scalp_rsi_ema1221_60_deltas_vs57.json",
        )
    ...
    if improve is not None and scalp_rsi_mr is not None:
        ...
        path_d59 = write_report_json(
            ...
            reports / "rise_panel_v1_scalp_rsi_ema1221_60_deltas_vs59.json",
        )
    ...
    if improve is not None:
        ...
        path_d58 = write_report_json(
            ...
            reports / "rise_panel_v1_scalp_rsi_ema1221_60_deltas_vs58.json",
        )
    ...
    if (
        ...
        cascade_bundle = run_cascade_compound_panel(
            cfg, data_dir=data_dir, pause_s=args.pause_s, scalp_mode="1h_rsi_ema1221"
        )
        path_c = write_cascade_json(
            cascade_bundle,
            reports / "rise_panel_v1_cascade_compound_scalp_rsi_ema1221_1h.json",
        )
        print(
            json.dumps(
                {
