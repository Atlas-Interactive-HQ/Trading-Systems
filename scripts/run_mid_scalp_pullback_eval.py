#!/usr/bin/env python3
"""Mid + Scalp EMA pullback long eval. Research only. Never places orders.
...
from atlas.paper.mid_scalp_pullback_eval import (  # noqa: E402
    render_markdown,
    run_set,
    write_report_json,
)
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Mid+Scalp pullback eval (research only)")
    ...
    p.add_argument("--write-md", default="phase1/35-mid-scalp-pullback.md")
    ...
    path_a = write_report_json(bundle_a, reports / "mid_scalp_pullback_A.json")
    ...
    if args.set == "B":
        ...
        path_b = write_report_json(bundle_b, reports / "mid_scalp_pullback_B.json")
        print(json.dumps({"wrote": str(path_b), "verdict": bundle_b["aggregate"]["verdict"]}, indent=2))
        existing_a = reports / "mid_scalp_pullback_A.json"
    ...
    summary = {
        ...
        "scalp_A": bundle_a["aggregate"]["scalp"]["pass"],
    }
