#!/usr/bin/env python3
"""Public-MD Scalp first scores #117 — EMA12/21 1H PUMP/TRUMP/WIF €20.

Research only. Never places orders. Does NOT change config/default.yaml.
not_a_forecast. Soft PASS ≠ arm. Method lock: phase1/115. No S1 transplant.
"""
...
from atlas.paper.public_md_scalp_first_score import (  # noqa: E402
    measured_table_rows,
    run_first_score,
    write_report_json,
)
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description=(
            "Public-MD Scalp #117 first scores: EMA12/21 1H long/flat "
    ...
    p.add_argument(
        "--out",
        default=None,
        help="JSON report path (default: data/reports/public_md_scalp_first_score_117.json)",
    )
    ...
    out_path = (
        Path(args.out)
        if args.out
        else reports / "public_md_scalp_first_score_117.json"
    )
    if not out_path.is_absolute():
        out_path = _ROOT / out_path

    try:
        bundle = run_first_score(cfg, data_dir=data_dir, pause_s=args.pause_s)
    ...
    results_path = _ROOT / "results" / "public_md_scalp_first_score_117.json"
