#!/usr/bin/env python3
"""Scalp DOGE EMA12/30 1H + daily bull Core-style RETURN gate eval. Research only. No orders.
...
from atlas.paper.scalp_doge_ema_1h_eval import (  # noqa: E402
    render_markdown,
    render_set_markdown,
    run_set,
    write_report_json,
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description="Scalp DOGE EMA12/30 1H + daily bull Core-style RETURN gate (research only)"
    )
    p.add_argument("--config", default=None)
    p.add_argument("--data-dir", default=None)
    p.add_argument("--pause-s", type=float, default=0.12)
    p.add_argument("--set", choices=("A", "B", "both"), default="both")
    p.add_argument("--write-md", default="phase1/48-scalp-doge-ema-1h.md")
    ...
    if args.set in ("A", "both"):
        ...
        path_a = write_report_json(bundle_a, reports / "scalp_doge_ema_1h_A.json")
        md_a = reports / "scalp_doge_ema_1h_A.md"
    ...
    if args.set in ("B", "both"):
        ...
        path_b = write_report_json(bundle_b, reports / "scalp_doge_ema_1h_B.json")
        md_b = reports / "scalp_doge_ema_1h_B.md"
    ...
    if bundle_a is None:
        existing_a = reports / "scalp_doge_ema_1h_A.json"
        if existing_a.is_file():
            bundle_a = json.loads(existing_a.read_text(encoding="utf-8"))
    if bundle_b is None:
        existing_b = reports / "scalp_doge_ema_1h_B.json"
