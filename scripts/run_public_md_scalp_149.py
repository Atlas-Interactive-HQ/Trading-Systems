#!/usr/bin/env python3
"""Public-MD Scalp #149 — B4/B5 (+B4b) 1D/4H EMA21 regime. Paper only.
...
from atlas.paper.public_md_scalp_149 import (  # noqa: E402
    OFFICIAL_CELLS,
    measured_table_rows,
    run_149_score,
    write_report_json,
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description=(
            "Public-MD Scalp #149: B4/B5(+B4b) notebook + EMA21 regime "
    ...
    p.add_argument(
        ...
        help="Comma-separated B4,B5,B4b (default all)",
    )
    p.add_argument(
        "--skip-b4b",
        action="store_true",
        help="Skip optional B4b (4H regime) cell",
    )
    ...
    out_path = (
        Path(args.out) if args.out else reports / "public_md_scalp_149.json"
    )
    ...
    def _norm(x: str) -> str:
        u = x.strip().upper()
        return "B4b" if u == "B4B" else u

    cells = tuple(_norm(x) for x in args.cells.split(",") if x.strip())
    try:
        bundle = run_149_score(
            cfg,
            data_dir=data_dir,
            results_dir=results_dir,
            cells=cells,
            include_b4b=not args.skip_b4b,
        )
    ...
    results_path = results_dir / "public_md_scalp_149.json"
    ...
    public = {
        ...
        "b4b_included": bundle.get("b4b_included"),
        "b4b_skip_reason": bundle.get("b4b_skip_reason"),
