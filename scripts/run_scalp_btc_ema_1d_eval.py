#!/usr/bin/env python3
"""Scalp BTC EMA12/30 1D Core-style RETURN gate eval (Scalp sleeve). Research only. No orders.

Does NOT change config/default.yaml. not_a_forecast. Mid/Core OUT.
SAME Core rule as Mid #51 on BTC at Scalp €20 on 1D. Same A/B calendars as Mid #51.
...
from atlas.paper.scalp_btc_ema_1d_eval import (  # noqa: E402
    render_markdown,
    render_set_markdown,
    run_set,
    write_report_json,
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Scalp BTC EMA12/30 1D Core-style RETURN gate (research only)")
    p.add_argument("--config", default=None)
    p.add_argument("--data-dir", default=None)
    p.add_argument("--pause-s", type=float, default=0.12)
    p.add_argument("--set", choices=("A", "B", "both"), default="both")
    p.add_argument("--write-md", default="phase1/53-scalp-btc-ema-1d.md")
    ...
    if args.set in ("A", "both"):
        ...
        path_a = write_report_json(bundle_a, reports / "scalp_btc_ema_1d_A.json")
        md_a = reports / "scalp_btc_ema_1d_A.md"
    ...
    if args.set in ("B", "both"):
        ...
        path_b = write_report_json(bundle_b, reports / "scalp_btc_ema_1d_B.json")
        md_b = reports / "scalp_btc_ema_1d_B.md"
    ...
    if bundle_a is None:
        existing_a = reports / "scalp_btc_ema_1d_A.json"
        if existing_a.is_file():
            bundle_a = json.loads(existing_a.read_text(encoding="utf-8"))
    if bundle_b is None:
        existing_b = reports / "scalp_btc_ema_1d_B.json"
