#!/usr/bin/env python3
"""rise_panel_v1 Core C2 — Donchian 40/20 + EMA50/200 vs Core EMA12/30 C0 on locked 7.

Research only. Never places orders. Does NOT change config/default.yaml. not_a_forecast.
Soft PASS ≠ Core-arm. Pre-registered after C1 FAIL — NOT rescue. If FAIL → no C3.
...
from atlas.paper.rise_panel_core_c2_donchian_ema_1d_eval import (  # noqa: E402
    CORE_C0_ID,
    CORE_C2_ID,
    deltas_vs_core_c0,
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description="rise_panel_v1 Core C2: Donchian 40/20 + EMA50/200 on 1D vs Core EMA C0"
    )
    ...
    p.add_argument(
        ...
        default="both",
        help="baseline=Core EMA C0; improve=Donchian+EMA50/200 C2; both=run + deltas",
    )
    p.add_argument(
        "--write-md",
        default="phase1/89-rise-panel-core-c2-donchian40-20-ema50-200-1d.md",
    )
    ...
    if args.mode in ("baseline", "both"):
        ...
        path_b = write_report_json(
            baseline, reports / "rise_panel_v1_core_c0_ema_recheck_c2.json"
        )
        print(
            json.dumps(
                {
    ...
    if args.mode in ("improve", "both"):
        improve = run_core_c2_donchian_ema(cfg, data_dir=data_dir, pause_s=args.pause_s)
        path_i = write_report_json(
            improve, reports / "rise_panel_v1_core_c2_donchian40_20_ema50_200_1d_89.json"
        )
        print(
            json.dumps(
                {
                    "wrote": str(path_i),
                    "core_c2_id": CORE_C2_ID,
                    "ok": improve.get("ok"),
    ...
    if args.mode == "improve" and baseline is None:
        existing = reports / "rise_panel_v1_core_c0_ema_recheck_c2.json"
        if existing.is_file():
    ...
    if baseline is not None and improve is not None:
        ...
        path_d = write_report_json(
            ...
            reports / "rise_panel_v1_core_c2_donchian_ema_89_deltas.json",
        )
            ...
            json.dumps(
                {
                    ...
                    "c3_allowed": (deltas or {}).get("c3_allowed"),
                },
                indent=2,
            )
        )
