#!/usr/bin/env python3
"""Mid BTC Donchian + EMA regime long eval. Research only. Never places orders.

Does NOT change config/default.yaml. not_a_forecast. Scalp OUT.
NEW family vs Mid daily pullback (#38 FAIL). No ATR stop.
"""
...
from atlas.paper.mid_btc_donchian_ema_eval import (  # noqa: E402
    render_markdown,
    run_set,
    write_report_json,
)
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Mid BTC Donchian+EMA eval (research only)")
    ...
    p.add_argument("--write-md", default="phase1/39-mid-btc-donchian-ema.md")
    ...
    path_a = write_report_json(bundle_a, reports / "mid_btc_donchian_ema_A.json")
    ...
    if args.set == "B":
        ...
        path_b = write_report_json(bundle_b, reports / "mid_btc_donchian_ema_B.json")
        print(json.dumps({"wrote": str(path_b), "verdict": bundle_b["aggregate"]["verdict"]}, indent=2))
        existing_a = reports / "mid_btc_donchian_ema_A.json"
