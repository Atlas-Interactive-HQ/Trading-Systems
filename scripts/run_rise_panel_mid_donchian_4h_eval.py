#!/usr/bin/env python3
"""rise_panel_v1 Mid #64 — Donchian 20/10 4H vs Mid EMA12/30 baseline on locked 7.
...
from atlas.paper.rise_panel_mid_donchian_4h_eval import (  # noqa: E402
    MID_BASELINE_ID,
    MID_IMPROVE_ID,
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description=(
            "rise_panel_v1 Mid #64: Donchian 20/10 on 4H vs Mid EMA baseline"
        )
    )
    ...
    p.add_argument(
        ...
        default="both",
        help="baseline=Mid EMA12/30 4H; improve=Donchian; both=run + deltas",
    )
    p.add_argument(
        "--skip-core-mid-book",
        action="store_true",
    ...
    p.add_argument("--write-md", default="phase1/64-rise-panel-mid-donchian20-10-4h.md")
    ...
    if args.mode in ("baseline", "both"):
        ...
        path_b = write_report_json(
            baseline, reports / "rise_panel_v1_mid_baseline_recheck_64.json"
        )
        print(
            json.dumps(
                {
    ...
    if args.mode in ("improve", "both"):
        improve = run_mid_donchian_4h(cfg, data_dir=data_dir, pause_s=args.pause_s)
        path_i = write_report_json(
            improve, reports / "rise_panel_v1_mid_donchian20_10_4h_64.json"
        )
        print(
            json.dumps(
                {
    ...
    if args.mode == "improve" and baseline is None:
        existing = reports / "rise_panel_v1_mid_baseline_recheck_64.json"
        if existing.is_file():
    ...
    if baseline is not None and improve is not None:
        ...
        path_d = write_report_json(
            ...
            reports / "rise_panel_v1_mid_donchian_64_deltas.json",
        )
    ...
    if soft_pass and improve is not None and not args.skip_core_mid_book:
        ...
        path_c = write_report_json(
            core_mid_book,
            reports / "rise_panel_v1_core_mid_book_mid_donchian_64.json",
        )
        print(
            json.dumps(
                {
    ...
    if args.write_md and baseline is not None and improve is not None:
        md = render_results_markdown(
            baseline, improve, deltas=deltas, core_mid_book=core_mid_book
        )
