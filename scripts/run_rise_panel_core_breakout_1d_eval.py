#!/usr/bin/env python3
"""rise_panel_v1 Core #69 — BreakoutV1 1D vs Core EMA12/30 baseline on locked 7.

Research only. Never places orders. Does NOT change config/default.yaml. not_a_forecast.
On PASS + better panel_net: Core(Breakout)+Mid(Breakout) book €180 (no Scalp).
...
from atlas.paper.rise_panel_core_breakout_1d_eval import (  # noqa: E402
    CORE_BASELINE_ID,
    CORE_IMPROVE_ID,
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description=(
            "rise_panel_v1 Core #69: BreakoutV1 on 1D vs Core EMA baseline"
        )
    )
    ...
    p.add_argument(
        ...
        default="both",
        help="baseline=Core EMA12/30 1D; improve=BreakoutV1; both=run + deltas",
    )
    p.add_argument(
        "--skip-core-mid-book",
        action="store_true",
    ...
    p.add_argument("--write-md", default="phase1/69-rise-panel-core-breakoutv1-1d.md")
    ...
    if args.mode in ("baseline", "both"):
        ...
        path_b = write_report_json(
            baseline, reports / "rise_panel_v1_core_ema_baseline_recheck_69.json"
        )
        print(
            json.dumps(
                {
    ...
    if args.mode in ("improve", "both"):
        improve = run_core_breakout_1d(cfg, data_dir=data_dir, pause_s=args.pause_s)
        path_i = write_report_json(
            improve, reports / "rise_panel_v1_core_breakoutv1_1d_69.json"
        )
        print(
            json.dumps(
                {
    ...
    if args.mode == "improve" and baseline is None:
        existing = reports / "rise_panel_v1_core_ema_baseline_recheck_69.json"
        if existing.is_file():
    ...
    if baseline is not None and improve is not None:
        ...
        path_d = write_report_json(
            ...
            reports / "rise_panel_v1_core_breakout_69_deltas.json",
        )
    ...
    if (
        ...
        path_c = write_report_json(
            core_mid_book,
            reports / "rise_panel_v1_core_mid_book_core_breakout_69.json",
        )
        print(
            json.dumps(
                {
