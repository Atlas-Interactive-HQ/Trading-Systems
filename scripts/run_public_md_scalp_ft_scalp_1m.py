#!/usr/bin/env python3
"""Public-MD Scalp #123 — freqtrade Scalp.py 1m Jul2020–Jan2021 €20.

GPL-cited reimpl. Never places orders. Does NOT change config/default.yaml.
not_a_forecast. Soft PASS ≠ arm. No PEPE. Do not edit phase1/120/121/122.
"""
...
from atlas.paper.public_md_scalp_ft_scalp_1m import (  # noqa: E402
    measured_table_rows,
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description=(
            "Public-MD Scalp #123: freqtrade Scalp.py 1m "
    ...
    p.add_argument(
        "--out",
        default=None,
        help="JSON report path (default: data/reports/public_md_scalp_ft_scalp_1m_123.json)",
    )
    ...
    out_path = (
        Path(args.out)
        if args.out
        else reports / "public_md_scalp_ft_scalp_1m_123.json"
    )
    if not out_path.is_absolute():
        out_path = _ROOT / out_path

    try:
        bundle = run_ft_scalp_1m_score(
            cfg,
            data_dir=data_dir,
    ...
    results_path = results_dir / "public_md_scalp_ft_scalp_1m_123.json"
    ...
    public = {
        ...
        "native_1m": bundle.get("native_1m"),
