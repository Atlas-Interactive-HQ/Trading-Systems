#!/usr/bin/env python3
"""Public-MD Scalp scores #119 — RSI14 MR 1H PUMP/TRUMP/WIF €20 (team-fit).

Research only. Never places orders. Does NOT change config/default.yaml.
not_a_forecast. Soft PASS ≠ arm. Method lock: phase1/115.
Complementary to trend board (#117 EMA / #118 Breakout). No S1 / no grind.
"""
...
from atlas.paper.public_md_scalp_rsi_mr_score import (  # noqa: E402
    measured_table_rows,
    run_rsi_mr_score,
    write_report_json,
)
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description=(
            "Public-MD Scalp #119 scores: RSI14 mean-reversion 1H long/flat "
            "PUMP/TRUMP/WIF-USDC €20. Research only. Team-fit complement."
        )
    )
    ...
    p.add_argument(
        "--out",
        default=None,
        help="JSON report path (default: data/reports/public_md_scalp_rsi_mr_score_119.json)",
    )
    ...
    out_path = (
        Path(args.out)
        if args.out
        else reports / "public_md_scalp_rsi_mr_score_119.json"
    )
    if not out_path.is_absolute():
        out_path = _ROOT / out_path

    try:
        bundle = run_rsi_mr_score(cfg, data_dir=data_dir, pause_s=args.pause_s)
    ...
    results_path = _ROOT / "results" / "public_md_scalp_rsi_mr_score_119.json"
