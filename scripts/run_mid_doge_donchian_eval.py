#!/usr/bin/env python3
"""Mid DOGE Donchian 20/10 long/flat eval (NO EMA). Research only. Never places orders.

Does NOT change config/default.yaml. not_a_forecast. Scalp OUT.
Always runs BOTH set A and set B before finalizing PASS/FAIL.
...
from atlas.paper.mid_doge_donchian_eval import (  # noqa: E402
    render_markdown,
    render_set_markdown,
    run_set,
    write_report_json,
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Mid DOGE Donchian eval (research only; NO EMA)")
    p.add_argument("--config", default=None)
    p.add_argument("--data-dir", default=None)
    p.add_argument("--pause-s", type=float, default=0.12)
    p.add_argument("--set", choices=("A", "B", "both"), default="both")
    p.add_argument("--write-md", default="phase1/42-mid-doge-donchian.md")
    ...
    if args.set in ("A", "both"):
        ...
        path_a = write_report_json(bundle_a, reports / "mid_doge_donchian_A.json")
        md_a = reports / "mid_doge_donchian_A.md"
    ...
    if args.set in ("B", "both"):
        ...
        path_b = write_report_json(bundle_b, reports / "mid_doge_donchian_B.json")
        md_b = reports / "mid_doge_donchian_B.md"
    ...
    # Combined phase1 md only when both available (or load missing from disk)
    if bundle_a is None:
        existing_a = reports / "mid_doge_donchian_A.json"
        if existing_a.is_file():
            bundle_a = json.loads(existing_a.read_text(encoding="utf-8"))
    if bundle_b is None:
        existing_b = reports / "mid_doge_donchian_B.json"
