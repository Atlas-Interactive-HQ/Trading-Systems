#!/usr/bin/env python3
"""Mid daily EMA pullback long eval. Research only. Never places orders.
...
from atlas.paper.mid_daily_pullback_eval import (  # noqa: E402
    render_markdown,
    run_set,
    write_report_json,
)
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Mid daily pullback eval (research only)")
    ...
    p.add_argument("--write-md", default="phase1/36-mid-daily-pullback.md")
    ...
    path_a = write_report_json(bundle_a, reports / "mid_daily_pullback_A.json")
    ...
    if args.set == "B":
        ...
        path_b = write_report_json(bundle_b, reports / "mid_daily_pullback_B.json")
        print(json.dumps({"wrote": str(path_b), "verdict": bundle_b["aggregate"]["verdict"]}, indent=2))
        existing_a = reports / "mid_daily_pullback_A.json"
    ...
    summary = {
        ...
        "low_freq_flag_A": bundle_a["aggregate"]["mid"].get("low_freq_flag"),
        "scalp": "OUT",
    }
