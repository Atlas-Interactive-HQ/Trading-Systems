#!/usr/bin/env python3
"""Public-MD Scalp #148 — Q4/Q5 2022+2023 OOS dual gate. Paper only.
...
from atlas.paper.public_md_scalp_148 import (  # noqa: E402
    OFFICIAL_CELLS,
    measured_table_rows,
    run_148_score,
    write_report_json,
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description=(
            "Public-MD Scalp #148: Q4/Q5 TRAIN+STRESS2021+OOS2022+OOS2023 "
    ...
    p.add_argument(
        ...
        help="Comma-separated Q4,Q5 (default both)",
    )
    p.add_argument(
        "--allow-repro-diverge",
        action="store_true",
        help="Do not fail-closed if TRAIN/2021 diverge from #147 (debug only)",
    )
    ...
    out_path = (
        Path(args.out) if args.out else reports / "public_md_scalp_148.json"
    )
    ...
    try:
        bundle = run_148_score(
            cfg,
            data_dir=data_dir,
            results_dir=results_dir,
            cells=cells,
            stop_on_repro_diverge=not args.allow_repro_diverge,
        )
    ...
    results_path = results_dir / "public_md_scalp_148.json"
    ...
    public = {
        ...
        "repro_147_all_match": bundle.get("repro_147_all_match"),
