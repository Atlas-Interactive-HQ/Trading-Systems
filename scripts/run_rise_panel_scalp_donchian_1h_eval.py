#!/usr/bin/env python3
"""rise_panel_v1 Scalp #61 — DOGE 1H Donchian 20/10 €20 vs provisional / 4H / RSI MR.
...
from atlas.paper.rise_panel_scalp_donchian_1h_eval import (  # noqa: E402
    SCALP_4H_ID,
    SCALP_IMPROVE_ID,
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        ...
        default="both",
        help="provisional=#55 1H; improve=Donchian 1H; both=run+deltas; refs=also re-score #57 4H",
    )
    p.add_argument("--write-md", default="phase1/61-rise-panel-scalp-donchian20-10-1h.md")
    p.add_argument(
        "--skip-cascade",
        action="store_true",
    ...
    if run_4h:
        ...
        path_4 = write_report_json(
            scalp_4h, reports / "rise_panel_v1_scalp_4h_recheck_for_61.json"
        )
        print(
            json.dumps(
                {
    ...
    if run_imp:
        improve = run_scalp_donchian_1h(cfg, data_dir=data_dir, pause_s=args.pause_s)
        path_i = write_report_json(
            improve, reports / "rise_panel_v1_scalp_donchian20_10_1h_61.json"
        )
        print(
            json.dumps(
                {
    ...
    # Honesty vs #59: prefer live report, else snapshot
    alt59 = Path(
    ...
    if provisional is not None and improve is not None:
        ...
        path_d = write_report_json(
            ...
            reports / "rise_panel_v1_scalp_donchian_61_deltas_vs55.json",
        )
    ...
    if scalp_4h is not None and improve is not None:
        ...
        path_d57 = write_report_json(
            ...
            reports / "rise_panel_v1_scalp_donchian_61_deltas_vs57.json",
        )
    ...
    if improve is not None and scalp_rsi_mr is not None:
        ...
        path_d59 = write_report_json(
            ...
            reports / "rise_panel_v1_scalp_donchian_61_deltas_vs59.json",
        )
    ...
    if (
        ...
        cascade_bundle = run_cascade_compound_panel(
            cfg, data_dir=data_dir, pause_s=args.pause_s, scalp_mode="1h_donchian"
        )
        path_c = write_cascade_json(
            cascade_bundle,
            reports / "rise_panel_v1_cascade_compound_scalp_donchian_1h.json",
        )
        print(
            json.dumps(
                {
