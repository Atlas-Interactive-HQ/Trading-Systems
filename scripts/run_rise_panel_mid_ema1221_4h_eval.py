#!/usr/bin/env python3
"""rise_panel_v1 Mid #67 — EMA12/21 4H vs Mid Breakout #65 baseline on locked 7.
...
from atlas.paper.rise_panel_mid_ema1221_4h_eval import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description=(
            "rise_panel_v1 Mid #67: EMA12/21 on 4H vs Mid Breakout #65 baseline"
        )
    )
    ...
    p.add_argument(
        ...
        default="both",
        help="baseline=Mid BreakoutV1 4H; improve=EMA12/21; both=run + deltas",
    )
    p.add_argument(
        "--skip-core-mid-book",
        action="store_true",
    ...
    p.add_argument("--write-md", default="phase1/67-rise-panel-mid-ema12-21-4h.md")
    ...
    if args.mode in ("baseline", "both"):
        ...
        path_b = write_report_json(
            baseline, reports / "rise_panel_v1_mid_breakout_baseline_recheck_67.json"
        )
        print(
            json.dumps(
                {
    ...
    if args.mode in ("improve", "both"):
        improve = run_mid_ema1221_4h(cfg, data_dir=data_dir, pause_s=args.pause_s)
        path_i = write_report_json(
            improve, reports / "rise_panel_v1_mid_ema12_21_4h_67.json"
        )
        print(
            json.dumps(
                {
    ...
    if args.mode == "improve" and baseline is None:
        existing = reports / "rise_panel_v1_mid_breakout_baseline_recheck_67.json"
        if existing.is_file():
    ...
    if baseline is not None and improve is not None:
        ...
        path_d = write_report_json(
            ...
            reports / "rise_panel_v1_mid_ema1221_67_deltas.json",
        )
        print(
            json.dumps(
                {
    ...
    if run_book and improve is not None:
        ...
        path_c = write_report_json(
            core_mid_book,
            reports / "rise_panel_v1_core_mid_book_mid_ema1221_67.json",
        )
        print(
            json.dumps(
                {
