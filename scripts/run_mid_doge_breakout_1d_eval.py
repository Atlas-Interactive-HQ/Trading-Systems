#!/usr/bin/env python3
"""Mid DOGE BreakoutV1 1D + same-bar EMA bull Core-style RETURN gate eval (Mid sleeve).

Research only. No orders. Does NOT change config/default.yaml. not_a_forecast.
...
from atlas.paper.mid_doge_breakout_1d_eval import (  # noqa: E402
    render_markdown,
    render_set_markdown,
    run_set,
    write_report_json,
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Mid DOGE BreakoutV1 1D Core-style RETURN gate (research only)")
    p.add_argument("--config", default=None)
    p.add_argument("--data-dir", default=None)
    p.add_argument("--pause-s", type=float, default=0.12)
    p.add_argument("--set", choices=("A", "B", "both"), default="both")
    p.add_argument("--write-md", default="phase1/47-mid-doge-breakout-1d.md")
    ...
    if args.set in ("A", "both"):
        ...
        path_a = write_report_json(bundle_a, reports / "mid_doge_breakout_1d_A.json")
        md_a = reports / "mid_doge_breakout_1d_A.md"
    ...
    if args.set in ("B", "both"):
        ...
        path_b = write_report_json(bundle_b, reports / "mid_doge_breakout_1d_B.json")
        md_b = reports / "mid_doge_breakout_1d_B.md"
    ...
    if bundle_a is None:
        existing_a = reports / "mid_doge_breakout_1d_A.json"
        if existing_a.is_file():
            bundle_a = json.loads(existing_a.read_text(encoding="utf-8"))
    if bundle_b is None:
        existing_b = reports / "mid_doge_breakout_1d_B.json"
    ...
    if bundle_a is not None:
        ...
        summary["clean_pass_windows_A"] = bundle_a["aggregate"]["mid"].get("clean_pass_windows")
        ...
        summary["n_trades_A"] = bundle_a["aggregate"]["mid"].get("n_trades_per_window")
        summary["tim_A"] = bundle_a["aggregate"]["mid"].get("tim_per_window")
        summary["expectancy_A"] = bundle_a["aggregate"]["mid"].get("expectancy_per_window")
    if bundle_b is not None:
        summary["mid_B"] = bundle_b["aggregate"]["mid"]["pass"]
        summary["clean_pass_windows_B"] = bundle_b["aggregate"]["mid"].get("clean_pass_windows")
        ...
        summary["expectancy_B"] = bundle_b["aggregate"]["mid"].get("expectancy_per_window")
