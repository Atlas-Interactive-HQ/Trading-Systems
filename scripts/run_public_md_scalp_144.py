...
not_a_forecast. Soft PASS N/A ≠ arm. Do not edit phase1/120–143.
"""
...
from atlas.paper.public_md_scalp_144 import (  # noqa: E402
    OFFICIAL_CELLS,
    measured_table_rows,
    run_144_score,
    write_report_json,
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description=(
            "Public-MD Scalp #144: S2 stretch T1 4R / T2 no-TP / T3 RVOL0.8 "
    ...
    p.add_argument(
        ...
        help="Comma-separated T1,T2,T3 (default all official)",
    )
    p.add_argument(
        "--s2-parent-json",
        default=None,
        help="Path to public_md_scalp_143.json for vs_s2 deltas",
    )
    ...
    out_path = (
        Path(args.out) if args.out else reports / "public_md_scalp_144.json"
    )
    if not out_path.is_absolute():
        out_path = _ROOT / out_path

    cells = tuple(x.strip().upper() for x in args.cells.split(",") if x.strip())
    s2_path = Path(args.s2_parent_json) if args.s2_parent_json else None
    try:
        bundle = run_144_score(
            cfg,
            data_dir=data_dir,
            results_dir=results_dir,
            include_subs=not args.full_only,
            cells=cells,
            s2_parent_json=s2_path,
        )
    ...
    results_path = results_dir / "public_md_scalp_144.json"
    ...
    public = {
        ...
        "s2_parent_cells_loaded": bundle.get("s2_parent_cells_loaded"),
