#!/usr/bin/env python3
"""Mid BTC EMA12/30 long-only eval (Core family; Mid sleeve). Research only. No orders.

Does NOT change config/default.yaml. not_a_forecast. Scalp OUT.
NOT pullback, NOT Donchian. Reuses ema_eval / EmaTrendV1 full-sleeve at €40.
"""
...
from atlas.paper.mid_btc_ema_long_eval import (  # noqa: E402
    render_markdown,
    run_set,
    write_report_json,
)
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Mid BTC EMA12/30 long-only eval (research only)")
    ...
    p.add_argument("--write-md", default="phase1/40-mid-btc-ema-long.md")
    ...
    path_a = write_report_json(bundle_a, reports / "mid_btc_ema_long_A.json")
    ...
    if args.set == "B":
        ...
        path_b = write_report_json(bundle_b, reports / "mid_btc_ema_long_B.json")
        print(json.dumps({"wrote": str(path_b), "verdict": bundle_b["aggregate"]["verdict"]}, indent=2))
        existing_a = reports / "mid_btc_ema_long_A.json"
    ...
    summary = {
        ...
        "zero_trade_windows_A": bundle_a["aggregate"]["mid"].get("zero_trade_windows"),
    ...
    if bundle_b is not None:
        ...
        summary["zero_trade_windows_B"] = bundle_b["aggregate"]["mid"].get("zero_trade_windows")
