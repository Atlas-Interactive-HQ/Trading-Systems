"""Unit tests: Mid DOGE EMA12/30 Core-style RETURN gate (#41)."""

from __future__ import annotations

from atlas.paper.cascade import MID_START_EUR
from atlas.paper.mid_doge_ema_coregate_eval import (
    BH_DD_MULT,
...
from atlas.strategy.ema_trend import FLAT, EmaTrendParams, EmaTrendV1
...
def test_locked_params():
    ...
    assert SOURCE == "mid_doge_ema_coregate"
...
def test_score_mid_abs_dd_fail_when_no_bh():
    full = {
        "ok": True,
        "n_trades": 1,
        "net_return_eur": 5.0,
...
def test_score_mid_holdout_zero_trade_low_tim_fail():
    ...
    hold = {
        "ok": True,
        "n_trades": 0,
        "net_return_eur": 1.0,  # positive but TIM low
        "max_dd_eur": 0.5,
        "bh_max_dd_eur": 0.5,
        "time_in_market": 0.1,
        "expectancy_after_costs_eur": None,
...
def test_aggregate_requires_two_full_and_holdouts():
    ...
    assert ok["mid"]["low_n_is_pass_gate"] is False
    assert ok["mid"]["differs_from_holdout_exp_gate"] is True

    # One clean + one holdout-red + one full-fail → only 1 clean → FAIL
    ...
    assert "A2" in fail_one_clean["mid"]["holdout_fail_windows"]

    # Two clean + one holdout-red → PASS (Core-style ≥2; extra does not veto)
    ...
    assert "A3" in pass_with_extra_red["mid"]["holdout_fail_windows"]
...
def test_not_using_expectancy_gate():
    ...
    full = {
        "ok": True,
        "n_trades": 3,
        "net_return_eur": -1.0,  # negative net
        "max_dd_eur": 2.0,
        "bh_max_dd_eur": 5.0,
        "time_in_market": 0.5,
        "expectancy_after_costs_eur": 0.5,  # positive exp — ignored by this gate
    }
    sc = score_mid(full, None)
    assert sc["gate_mode"] == "core_style_return"
    assert sc["full_pass"] is False
