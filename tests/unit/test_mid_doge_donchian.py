"""Unit tests: Mid DOGE Donchian 20/10 — NO EMA; holdout-exp gate like #36."""
...
from atlas.paper.mid_doge_donchian_eval import (
    LOW_FREQ_MEDIAN_CAP,
    MID_DD_CAP_EUR,
    MID_TIME_STOP_BARS,
    SPOT_MD,
...
def test_spot_md_is_doge_locked_params():
...
def test_no_ema_in_params_or_label():
    p = MidDogeDonchianParams()
    assert not hasattr(p, "ema_fast")
    strat = MidDogeDonchianV1()
...
def test_entry_on_donchian_breakout_without_ema():
    ...
    assert sig.reason == "mid_doge_donchian_breakout"
...
def test_no_entry_without_breakout():
    ...
    assert MidDogeDonchianV1().on_closed_bar(bars) is None


def test_no_short_ever():
    bars = _build_breakout_series()
    sig = MidDogeDonchianV1().on_closed_bar(bars)
...
def test_aggregate_low_freq_is_flag_not_hard_gate():
    ...
    # High median trades FLAG but still PASS (unlike #39 hard gate)
...
def test_engine_roundtrip_no_atr_tp_no_ema():
    ...
    eng = PaperEngine(settings, strat, journal=NullJournal(), run_id="test-doge-donch", data_dir="data")
