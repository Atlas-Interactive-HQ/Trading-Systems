"""Unit tests: rise_panel Scalp #60 RSI∩EMA12/21 1H lock + soft-promote deltas."""
...
from atlas.paper.rise_panel_scalp_rsi_ema1221_1h_eval import (
    SCALP_4H_ID,
    SCALP_IMPROVE_ID,
...
def test_ids_and_sleeve_locked():
    ...
    assert SCALP_IMPROVE_ID == "rise_panel_v1_scalp_doge_rsi14_mr_ema1221_1h_eur20"
    assert SCALP_START_EUR == 20.0
    assert PANEL_LABEL == "rise_panel_v1"
    assert len(RISE_PANEL_V1) == 7


def test_family_combined_rsi_ema1221():
    assert FAMILY == "rsi14_mr_ema1221_long_flat_1h"
    assert BAR == "1H"
    ...
    assert EMA_SLOW == 21
    assert SLEEVE == "scalp"
    s = ScalpDogeRsiEma1221V1()
    assert "scalp_doge_rsi_ema1221_1h" in s.label
    assert "ema12_21" in s.label or "ema12_21" in s.label.replace("__", "_")
    assert s.warmup_bars() == max(EMA_SLOW, RSI_PERIOD + 3)


def test_rejects_period_threshold_ema_tf_sleeve_sweeps():
    with pytest.raises(ValueError, match="period"):
        ScalpDogeRsiEma1221V1(ScalpDogeRsiEma1221Params(rsi_period=7))
    with pytest.raises(ValueError, match="threshold"):
        ScalpDogeRsiEma1221V1(ScalpDogeRsiEma1221Params(entry_rsi=25.0))
    with pytest.raises(ValueError, match="threshold"):
        ScalpDogeRsiEma1221V1(ScalpDogeRsiEma1221Params(exit_rsi=50.0))
    with pytest.raises(ValueError, match="EMA period"):
        ScalpDogeRsiEma1221V1(ScalpDogeRsiEma1221Params(ema_slow=30))
    with pytest.raises(ValueError, match="EMA period"):
        ScalpDogeRsiEma1221V1(ScalpDogeRsiEma1221Params(ema_fast=10))
    with pytest.raises(ValueError, match="bar"):
        ScalpDogeRsiEma1221V1(ScalpDogeRsiEma1221Params(bar="15m"))
    with pytest.raises(ValueError, match="sleeve"):
        ScalpDogeRsiEma1221V1(ScalpDogeRsiEma1221Params(sleeve="mid"))
...
def test_combined_can_go_long_and_flat():
    closes: list[float] = [100.0] * 25
    for i in range(12):
        closes.append(100.0 - 3.0 * (i + 1))
    for i in range(20):
        closes.append(closes[-1] + 2.8)
    for i in range(25):
