"""Unit tests: Scalp DOGE EMA12/30 1H + daily bull Core-style RETURN gate (#48)."""

from __future__ import annotations

from atlas.paper.cascade import SCALP_START_EUR
from atlas.paper.scalp_doge_ema_1h_eval import (
    BAR,
    BH_DD_MULT,
...
from atlas.strategy.scalp_doge_ema_1h import ScalpDogeEma1hParams, ScalpDogeEma1hV1
...
def b1h(i: int, c: float, symbol="DOGE-USDT") -> Bar:
    ts = START + i * BAR_MS
...
def test_locked_params():
    ...
    assert SOURCE == "scalp_doge_ema_1h"
...
def test_strategy_never_short_and_label():
    strat = ScalpDogeEma1hV1(ScalpDogeEma1hParams(fast=12, slow=30), daily_bars=rising_daily())
    assert strat.label.startswith("scalp_doge_ema_1h")
...
def test_daily_bull_true_on_rising_false_on_falling():
    ...
    s_bull = ScalpDogeEma1hV1(daily_bars=rising)
    s_bear = ScalpDogeEma1hV1(daily_bars=falling)
...
def test_entry_gate_blocks_new_long_when_daily_bear():
    ...
    strat = ScalpDogeEma1hV1(daily_bars=daily)
