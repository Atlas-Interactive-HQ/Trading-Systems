"""Unit tests for #150 D1/D2/D3 regime soften (no network, no invented trades)."""

from __future__ import annotations

from atlas.paper.public_md_scalp_150 import (
...
def test_official_cells_d1_d2_d3_params() -> None:
    assert OFFICIAL_CELLS == ("D1", "D2", "D3")
    assert CELL_R_MULTIPLE["D1"] == CELL_R_MULTIPLE["D2"] == CELL_R_MULTIPLE["D3"] == 4.0
    assert CELL_REGIME_MODE["D1"] == "entry_only"
    assert CELL_REGIME_MODE["D2"] == "sticky3"
    assert CELL_REGIME_MODE["D3"] == "ema50_flip"
    assert CELL_EMA_PERIOD["D1"] == 21
    assert CELL_EMA_PERIOD["D2"] == 21
    assert CELL_EMA_PERIOD["D3"] == 50
    assert CELL_STICKY_N["D2"] == 3
    assert CELL_STICKY_N["D1"] is None and CELL_STICKY_N["D3"] is None
    assert CELL_ALLOW_REGIME_EXIT["D1"] is False
    assert CELL_ALLOW_REGIME_EXIT["D2"] is True
    assert CELL_ALLOW_REGIME_EXIT["D3"] is True
...
def test_d1_allow_regime_exit_false() -> None:
    assert CELL_ALLOW_REGIME_EXIT["D1"] is False
    assert CELL_REGIME_MODE["D1"] == "entry_only"


def test_d2_sticky_three() -> None:
    assert CELL_STICKY_N["D2"] == 3
    assert CELL_REGIME_MODE["D2"] == "sticky3"
    assert callable(sticky_regime_exit_1h_open_ms)


def test_d3_ema50() -> None:
    assert CELL_EMA_PERIOD["D3"] == 50
    assert CELL_REGIME_MODE["D3"] == "ema50_flip"
